...
vec_basic generate_fdiff_weights_vector(const vec_basic& grid, const unsigned max_deriv, const RCP<const Basic> around){
    ...
    // Returns
    // -------
    // weights[grid_index, deriv_order]: weights of order 0 to max_deriv (column major order)
    //
    // References
    // ----------
    ...
    for (unsigned i=1; i < len_g; ++i){
        ...
        for (unsigned j=0; j<i; ++j){
            ...
            if (j == i-1){
                for (int k=mn; k>=1; --k){
                    weights[i + k*len_g] = div(mul(c1, sub(mul(integer(k), weights[i - 1 + (k-1)*len_g]),
                                                           mul(c5, weights[i - 1 + k*len_g]))
                                                   ), c2);
                }
                weights[i] = mul(minus_one, div(mul(c1, mul(c5, weights[i-1])), c2));
            }
            for (int k=mn; k>=1; --k){
                weights[j + k*len_g] = div(sub(mul(c4, weights[j + k*len_g]), mul(integer(k), weights[j + (k-1)*len_g])), c3);
            }
