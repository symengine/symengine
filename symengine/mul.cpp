        ...
        const RCP<const Basic> &t)
{
    ...
    if (it == d.end()) {
        ...
        if (is_a_Number(*it->second) && is_a_Number(*exp)) {
            RCP<const Number> tmp = addnum(rcp_static_cast<const Number>(it->second),
                                           rcp_static_cast<const Number>(exp));
            if (tmp->is_exact_zero()) {
                d.erase(it);
            } else {
                it->second = tmp;
            }
            ...
            if (is_a_Number(*it->second) &&
                    rcp_static_cast<const Number>(it->second)->is_exact_zero()) {
                d.erase(it);
            }
        }
    }
    ...
    const RCP<const Basic> &exp, const RCP<const Basic> &t)
{
    ...
    if (it == d.end()) {
        ...
        if (is_a<Integer>(*it->second)) {
            ...
            imulnum(outArg(*coef), get_one_from_zero(rcp_static_cast<const Number>(it->second)));
            d.erase(it);
        }
    }
}
...
RCP<const Basic> Mul::power_all_terms(const RCP<const Basic> &exp) const
{
    if (is_a_Number(*exp) && rcp_static_cast<const Number>(exp)->is_zero()) {
        ...
        return get_one_from_zero(rcp_static_cast<const Number>(exp));
    }
    SymEngine::map_basic_basic d;
