...
bool get_pi_shift(const RCP<const Basic> &arg, const Ptr<RCP<const Integer>> &n, const Ptr<RCP<const Basic>> &x)
{
    if (is_a<Add>(*arg)) {
        ...
        if (size > 1) {
            ...
            *x = coef;
            for (const auto &p : s.dict_) {
                ...
                if (is_a<Constant>(*p.first) and eq(*(rcp_static_cast<const Constant>(p.first)), *pi)
        ...
        if (s.dict_.size() == 1 and is_a<Constant>(*p->first) and eq(*(rcp_static_cast<const Constant>(p->first)), *pi)
            and eq(*(rcp_static_cast<const Number>(p->second)), *one) and is_a<Integer>(*coef)) {
    ...
    } else if (is_a<Constant>(*arg) and eq(*(rcp_static_cast<const Constant>(arg)), *pi)) {
        *n = integer(12);
        *x = zero;
        return true;
    } else {
...
bool could_extract_minus(const RCP<const Basic> &arg)
{
    if (is_a<Mul>(*arg)) {
        const Mul &s = static_cast<const Mul &>(*arg);
        RCP<const Basic> coef = s.coef_;
        if (is_a<Integer>(*coef) and rcp_static_cast<const Integer>(coef)->is_negative())
            return true;

        else if (is_a<Rational>(*coef) and rcp_static_cast<const Rational>(coef)->is_negative())
            return true;
        else
            return false;
...
Sin::Sin(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Sin>(o) and eq(*get_arg(), *(static_cast<const Sin &>(o).get_arg())))
        return true;
    return false;
}
...
Cos::Cos(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Cos>(o) and eq(*get_arg(), *(static_cast<const Cos &>(o).get_arg())))
        return true;
    return false;
}
...
Tan::Tan(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Tan>(o) and eq(*get_arg(), *(static_cast<const Tan &>(o).get_arg())))
        return true;
    else
        return false;
}
...
RCP<const Basic> tan(const RCP<const Basic> &arg)
{
    ...
    if (conjugate) {
        ...
        if (eq(*ret_arg, *zero)) {
            return mul(integer(sign), div(sin_table[index], sin_table[(index + 6) % 24]));
        } else {
            if (sign == 1) {
                if (neq(*ret_arg, *arg)) {
                    return tan(ret_arg);
...
Cot::Cot(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Cot>(o) and eq(*get_arg(), *(static_cast<const Cot &>(o).get_arg())))
        return true;
    else
        return false;
}
...
RCP<const Basic> cot(const RCP<const Basic> &arg)
{
    ...
    if (conjugate) {
        ...
        if (eq(*ret_arg, *zero)) {
            return mul(integer(sign), div(sin_table[(index + 6) % 24], sin_table[index]));
        } else {
            if (sign == 1) {
                if (neq(*ret_arg, *arg)) {
                    return cot(ret_arg);
...
Csc::Csc(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Csc>(o) and eq(*get_arg(), *(static_cast<const Csc &>(o).get_arg())))
        return true;
    return false;
}
...
Sec::Sec(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Sec>(o) and eq(*get_arg(), *(static_cast<const Sec &>(o).get_arg())))
        return true;
    return false;
}
...
/* ---------------------------- */
ASin::ASin(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ASin>(o) and eq(*get_arg(), *(static_cast<const ASin &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ACos::ACos(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ACos>(o) and eq(*get_arg(), *(static_cast<const ACos &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ASec::ASec(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ASec>(o) and eq(*get_arg(), *(static_cast<const ASec &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ACsc::ACsc(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ACsc>(o) and eq(*get_arg(), *(static_cast<const ASec &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ATan::ATan(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ATan>(o) and eq(*get_arg(), *(static_cast<const ATan &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ACot::ACot(const RCP<const Basic> &arg) : TrigFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ACot>(o) and eq(*get_arg(), *(static_cast<const ACot &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ATan2::ATan2(const RCP<const Basic> &num, const RCP<const Basic> &den) : num_{num}, den_{den}
{
    SYMENGINE_ASSERT(is_canonical(num, den))
}

bool ATan2::is_canonical(const RCP<const Basic> &num, const RCP<const Basic> &den) const
{
...
RCP<const Basic> Sin::create(const RCP<const Basic> &arg) const { return sin(arg); }

RCP<const Basic> Cos::create(const RCP<const Basic> &arg) const { return cos(arg); }

RCP<const Basic> Tan::create(const RCP<const Basic> &arg) const { return tan(arg); }

RCP<const Basic> Cot::create(const RCP<const Basic> &arg) const { return cot(arg); }

RCP<const Basic> Sec::create(const RCP<const Basic> &arg) const { return sec(arg); }

RCP<const Basic> Csc::create(const RCP<const Basic> &arg) const { return csc(arg); }

RCP<const Basic> ASin::create(const RCP<const Basic> &arg) const { return asin(arg); }

RCP<const Basic> ACos::create(const RCP<const Basic> &arg) const { return acos(arg); }

RCP<const Basic> ATan::create(const RCP<const Basic> &arg) const { return atan(arg); }

RCP<const Basic> ACot::create(const RCP<const Basic> &arg) const { return acot(arg); }

RCP<const Basic> ASec::create(const RCP<const Basic> &arg) const { return asec(arg); }

RCP<const Basic> ACsc::create(const RCP<const Basic> &arg) const { return acsc(arg); }

/* ---------------------------- */
LambertW::LambertW(const RCP<const Basic> &arg) : arg_{arg} { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<LambertW>(o) and eq(*arg_, *(static_cast<const LambertW &>(o).arg_)))
        return true;
    return false;
}
...
FunctionSymbol::FunctionSymbol(std::string name, const RCP<const Basic> &arg) : name_{name}, arg_{{arg}}
{
    SYMENGINE_ASSERT(is_canonical(arg_))
}

FunctionSymbol::FunctionSymbol(std::string name, const vec_basic &arg) : name_{name}, arg_{arg}
{
    SYMENGINE_ASSERT(is_canonical(arg_))
}

bool FunctionSymbol::is_canonical(const vec_basic &arg) const { return true; }
...
{
    if (is_a<FunctionSymbol>(o) and name_ == static_cast<const FunctionSymbol &>(o).name_
...
FunctionWrapper::FunctionWrapper(std::string name, const RCP<const Basic> &arg) : FunctionSymbol(name, arg) {}

FunctionWrapper::FunctionWrapper(std::string name, const vec_basic &vec) : FunctionSymbol(name, vec) {}

/* ---------------------------- */

Derivative::Derivative(const RCP<const Basic> &arg, const multiset_basic &x) : arg_{arg}, x_{x}
{
    SYMENGINE_ASSERT(is_canonical(arg, x))
}

bool Derivative::is_canonical(const RCP<const Basic> &arg, const multiset_basic &x) const
{
...
{
    if (is_a<Derivative>(o) and eq(*arg_, *(static_cast<const Derivative &>(o).arg_))
    ...
    for (const auto &p : subs_dict) {
        ...
        for (const auto &d : x_) {
            if (is_a<Symbol>(*d)) {
                ...
                if (neq(*zero, *(p.first->diff(s))) || neq(*zero, *(p.second->diff(s)))) {
                    subs = false;
                    break;
                }
            } else {
...
// Subs class
Subs::Subs(const RCP<const Basic> &arg, const map_basic_basic &dict) : arg_{arg}, dict_{dict}
{
    SYMENGINE_ASSERT(is_canonical(arg, dict))
}

bool Subs::is_canonical(const RCP<const Basic> &arg, const map_basic_basic &dict) const
{
...
Sinh::Sinh(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Sinh>(o) and eq(*get_arg(), *(static_cast<const Sinh &>(o).get_arg())))
        return true;
    else
        return false;
}
...
Csch::Csch(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Csch>(o) and eq(*get_arg(), *(static_cast<const Csch &>(o).get_arg())))
        return true;
    else
        return false;
}
...
Cosh::Cosh(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Cosh>(o) and eq(*get_arg(), *(static_cast<const Cosh &>(o).get_arg())))
        return true;
    else
        return false;
}
...
Sech::Sech(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Sech>(o) and eq(*get_arg(), *(static_cast<const Sech &>(o).get_arg())))
        return true;
    else
        return false;
}
...
Tanh::Tanh(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Tanh>(o) and eq(*get_arg(), *(static_cast<const Tanh &>(o).get_arg())))
        return true;
    else
        return false;
}
...
Coth::Coth(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<Coth>(o) and eq(*get_arg(), *(static_cast<const Coth &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ASinh::ASinh(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ASinh>(o) and eq(*get_arg(), *(static_cast<const ASinh &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ACsch::ACsch(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ACsch>(o) and eq(*get_arg(), *(static_cast<const ACsch &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ACosh::ACosh(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ACosh>(o) and eq(*get_arg(), *(static_cast<const ACosh &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ATanh::ATanh(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ATanh>(o) and eq(*get_arg(), *(static_cast<const ATanh &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ACoth::ACoth(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ACoth>(o) and eq(*get_arg(), *(static_cast<const ACoth &>(o).get_arg())))
        return true;
    else
        return false;
}
...
ASech::ASech(const RCP<const Basic> &arg) : HyperbolicFunction(arg) { SYMENGINE_ASSERT(is_canonical(arg)) }
...
{
    if (is_a<ASech>(o) and eq(*get_arg(), *(static_cast<const ASech &>(o).get_arg())))
        return true;
    else
        return false;
}
...
RCP<const Basic> Sinh::create(const RCP<const Basic> &arg) const { return sinh(arg); }

RCP<const Basic> Csch::create(const RCP<const Basic> &arg) const { return csch(arg); }

RCP<const Basic> Cosh::create(const RCP<const Basic> &arg) const { return cosh(arg); }

RCP<const Basic> Sech::create(const RCP<const Basic> &arg) const { return sech(arg); }

RCP<const Basic> Tanh::create(const RCP<const Basic> &arg) const { return tanh(arg); }

RCP<const Basic> Coth::create(const RCP<const Basic> &arg) const { return coth(arg); }

RCP<const Basic> ASinh::create(const RCP<const Basic> &arg) const { return asinh(arg); }

RCP<const Basic> ACsch::create(const RCP<const Basic> &arg) const { return acsch(arg); }

RCP<const Basic> ACosh::create(const RCP<const Basic> &arg) const { return acosh(arg); }

RCP<const Basic> ATanh::create(const RCP<const Basic> &arg) const { return atanh(arg); }

RCP<const Basic> ACoth::create(const RCP<const Basic> &arg) const { return acoth(arg); }

RCP<const Basic> ASech::create(const RCP<const Basic> &arg) const { return asech(arg); }

KroneckerDelta::KroneckerDelta(const RCP<const Basic> &i, const RCP<const Basic> &j) : i_{i}, j_{j}
{
...
{
    if (is_a<KroneckerDelta>(o) and eq(*i_, *(static_cast<const KroneckerDelta &>(o).i_))
...
LeviCivita::LeviCivita(const vec_basic &&arg) : arg_{std::move(arg)} { SYMENGINE_ASSERT(is_canonical(arg_)) }
...
{
    if (is_a<LeviCivita>(o) and vec_basic_eq(arg_, static_cast<const LeviCivita &>(o).arg_))
        return true;
    else
        return false;
}
...
Zeta::Zeta(const RCP<const Basic> &s) : s_{s}, a_{one} { SYMENGINE_ASSERT(is_canonical(s_, a_)) }
...
{
    if (is_a<Zeta>(o) and eq(*s_, *(static_cast<const Zeta &>(o).s_)) and eq(*a_, *(static_cast<const Zeta &>(o).a_)))
        return true;
    return false;
}
...
RCP<const Basic> zeta(const RCP<const Basic> &s) { return zeta(s, one); }

Dirichlet_eta::Dirichlet_eta(const RCP<const Basic> &s) : s_{s} { SYMENGINE_ASSERT(is_canonical(s_)) }
...
{
    if (is_a<Dirichlet_eta>(o) and eq(*s_, *(static_cast<const Dirichlet_eta &>(o).s_)))
        return true;
    return false;
}
...
RCP<const Basic> Dirichlet_eta::rewrite_as_zeta() const { return mul(sub(one, pow(i2, sub(one, s_))), zeta(s_)); }
...
Gamma::Gamma(const RCP<const Basic> &arg) : arg_{arg} { SYMENGINE_ASSERT(is_canonical(arg_)) }

bool Gamma::is_canonical(const RCP<const Basic> &arg) const
{
    if (is_a<Integer>(*arg))
        return false;
    if (is_a<Rational>(*arg) and (get_den(rcp_static_cast<const Rational>(arg)->i)) == 2) {
        return false;
    }
    if (is_a_Number(*arg) and not static_cast<const Number &>(*arg).is_exact()) {
        return false;
...
LowerGamma::LowerGamma(const RCP<const Basic> &s, const RCP<const Basic> &x) : s_{s}, x_{x}
{
...
{
    if (is_a<LowerGamma>(o) and eq(*s_, *(static_cast<const LowerGamma &>(o).s_))
...
UpperGamma::UpperGamma(const RCP<const Basic> &s, const RCP<const Basic> &x) : s_{s}, x_{x}
{
    SYMENGINE_ASSERT(is_canonical(s_, x_))
}
...
{
    if (is_a<UpperGamma>(o) and eq(*s_, *(static_cast<const UpperGamma &>(o).s_))
...
bool LogGamma::is_canonical(const RCP<const Basic> &arg) const
{
    if (is_a<Integer>(*arg)) {
        ...
        if (eq(*integer(1), *arg_int) or eq(*integer(2), *arg_int) or eq(*integer(3), *arg_int)) {
            return false;
        }
    }
    return true;
...
{
    if (is_a<LogGamma>(o) and eq(*arg_, *(static_cast<const LogGamma &>(o).arg_)))
        return true;
    return false;
}
...
RCP<const Basic> LogGamma::rewrite_as_gamma() const { return log(gamma(arg_)); }
...
bool Beta::is_canonical(const RCP<const Basic> &x, const RCP<const Basic> &y)
{
    ...
    if (is_a<Integer>(*x) or (is_a<Rational>(*x) and (get_den(rcp_static_cast<const Rational>(x)->i)) == 2)) {
        if (is_a<Integer>(*y) or (is_a<Rational>(*y) and (get_den(rcp_static_cast<const Rational>(y)->i)) == 2)) {
            return false;
        }
    }
    return true;
...
{
    if (is_a<Beta>(o)) {
        if (eq(*x_, *(static_cast<const Beta &>(o).x_)) and eq(*y_, *(static_cast<const Beta &>(o).y_))) {
            return true;
        }
    }
    return false;
...
Abs::Abs(const RCP<const Basic> &arg) : arg_{arg} { SYMENGINE_ASSERT(is_canonical(arg_)) }
...
Max::Max(const vec_basic &&arg) : arg_{std::move(arg)} { SYMENGINE_ASSERT(is_canonical(arg_)) }
...
{
    if (is_a<Max>(o) and vec_basic_eq_perm(arg_, static_cast<const Max &>(o).arg_))
        return true;
    else
        return false;
}
...
Min::Min(const vec_basic &&arg) : arg_{std::move(arg)} { SYMENGINE_ASSERT(is_canonical(arg_)) }
...
{
    if (is_a<Min>(o) and vec_basic_eq_perm(arg_, static_cast<const Min &>(o).arg_))
        return true;
    else
        return false;
}
