          ...
          "[matrices]")
{
    ...
    // Here the diagonal entry is 6 although the det(A) = -6, this
    // because we
          ...
          "[matrices]")
{
    ...
    // Solve two systems at once, Ax = transpose([7, 3]) and Ax =
    // transpose([4,
    // 6])
    A = DenseMatrix(2, 2, {integer(1), integer(1), integer(1), integer(-1)});
...
{
    // Example 3, page 14, Nakos, G. C., Turner, P. R., Williams, R. M.
    // (1997).
    // Fraction-free algorithms for linear and polynomial equations.
    ...
    // To solve the system Ax = b, We call forward substitution
    // algorithm with
    // LU and b. Then we call backward substitution algorithm with with
    // LU and
    // modified b from forward substitution. This will find the
    // solutions.
...
{
    DenseMatrix A = DenseMatrix(3, 3, {integer(4), integer(12),
integer(-16),
                                       integer(12), integer(37),
integer(-43),
