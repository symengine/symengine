...
TEST_CASE("eval_double: eval_double", "[eval_double]")
{
    ...
    std::vector<std::pair<RCP<const Basic>, double>> vec = {
        ...
        {add(add(add(asinh(r4), add(acosh(r4), add(atanh(r3), acoth(r4)))), csch(r4)), acsch(r3)), 7.336249966045},
        {add(add(sinh(one), add(cosh(one), add(tanh(one), coth(one)))), sech(r3)), 5.773239267559},
        {sub(add(add(asinh(r4), add(acosh(r4), add(atanh(r3), acoth(r4)))), sech(r4)), acsch(r4)), 4.825120290814},
    ...
    CHECK_THROWS_AS(eval_double_single_dispatch(*symbol("x")), std::runtime_error);
    ...
    CHECK_THROWS_AS(eval_double_single_dispatch(*levi_civita({r1})), std::runtime_error);

    CHECK_THROWS_AS(eval_double(*zeta(r1, r2)), std::runtime_error);
    CHECK_THROWS_AS(eval_double_single_dispatch(*zeta(r1, r2)), std::runtime_error);

    CHECK_THROWS_AS(eval_double(*constant("dummy")), std::runtime_error);
    CHECK_THROWS_AS(eval_double_single_dispatch(*constant("dummy")), std::runtime_error);
...
TEST_CASE("eval_complex_double: eval_double", "[eval_double]")
{
    ...
    std::vector<std::pair<RCP<const Basic>, std::complex<double>>> vec
        = {{r1, std::complex<double>(0.0, 4.624795545470)},
           ...
           {add(add(add(sinh(r3), add(neg(cosh(r4)), add(tanh(r3), neg(coth(r4))))), csch(r3)), sech(r4)),
            std::complex<double>(1.8376773309548138, 0.1756408412537389)},
           {add(add(asinh(r3), add(neg(acosh(r4)), add(atanh(r3), neg(acoth(r4))))), acsch(r3)),
            std::complex<double>(0.71589877741418859, 0.28103490150281308)},
           {log(div(pi, mul(E, r1))), std::complex<double>(-1.38670227775307, -1.57079632679490)},
