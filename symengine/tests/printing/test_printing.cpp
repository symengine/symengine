...
TEST_CASE("test_printing(): printing", "[printing]")
{
    ...
    CHECK(r->__str__() == "(23/6)*sqrt(2)*sqrt(3)*sqrt(5)*sqrt(7)");

    r = exp(symbol("x"));
    CHECK(r->__str__() == "exp(x)");
    r = mul(exp(symbol("x")), integer(10));
    CHECK(r->__str__() == "10*exp(x)");
    r = exp(mul(symbol("x"), integer(-1)));
    CHECK(r->__str__() == "exp(-x)");
    r = exp(integer(-1));
    CHECK(r->__str__() == "exp(-1)");
    r = mul(exp(integer(-1)), integer(2));
    CHECK(r->__str__() == "2*exp(-1)");
    r = mul(exp(integer(-3)), integer(2));
    CHECK(r->__str__() == "2*exp(-3)");
    r = mul(exp(integer(1)), integer(2));
    CHECK(r->__str__() == "2*E");
    r = div(exp(integer(-1)), symbol("x"));
    CHECK(r->__str__() == "exp(-1)/x");

    r = pow(div(symbol("x"), integer(2)), div(integer(1), integer(2)));
    CHECK(r->__str__() == "(1/2)*sqrt(2)*sqrt(x)");

    r = pow(div(integer(3), integer(2)), div(integer(1), integer(2)));
    CHECK(r->__str__() == "(1/2)*sqrt(2)*sqrt(3)");
    ...
    r1 = mul(c1, x);
    r2 = mul(c2, x);
    CHECK(c1->__str__() == "1/2 + 5/7*I");
    CHECK(c2->__str__() == "1/2 - 5/7*I");
    CHECK(r1->__str__() == "(1/2 + 5/7*I)*x");
    CHECK(r2->__str__() == "(1/2 - 5/7*I)*x");
    r1 = pow(x, c1);
    r2 = pow(x, c2);
    CHECK(r1->__str__() == "x**(1/2 + 5/7*I)");
    CHECK(r2->__str__() == "x**(1/2 - 5/7*I)");
    ...
    r1 = mul(c1, x);
    r2 = mul(c2, x);
    CHECK(c1->__str__() == "5/7*I");
    CHECK(c2->__str__() == "-5/7*I");
    CHECK(r1->__str__() == "5/7*I*x");
    CHECK(r2->__str__() == "-5/7*I*x");
    r1 = pow(x, c1);
    r2 = pow(x, c2);
    CHECK(r1->__str__() == "x**(5/7*I)");
    CHECK(r2->__str__() == "x**(-5/7*I)");

    c1 = Complex::from_two_nums(*rn2, *rn1);
    c2 = Complex::from_two_nums(*rn3, *rn1);
    r1 = mul(c1, x);
    r2 = mul(c2, x);
    CHECK(c1->__str__() == "5/7");
    CHECK(c2->__str__() == "-5/7");
    CHECK(r1->__str__() == "(5/7)*x");
    CHECK(r2->__str__() == "(-5/7)*x");
    r1 = pow(x, c1);
    r2 = pow(x, c2);
    CHECK(r1->__str__() == "x**(5/7)");
    CHECK(r2->__str__() == "x**(-5/7)");
    ...
    CHECK(r->__str__() == "Derivative(f(x), x)");
    CHECK(r1->__str__() == "Derivative(f(x), x)");
    CHECK(r2->__str__() == "Derivative(g(x), x)");
    CHECK(r1->compare(*r2) == -1);

    r1 = f->diff(x)->diff(x);
    CHECK(r1->__str__() == "Derivative(f(x), x, x)");

    f = function_symbol("f", {x, y});
    r = f->diff(x)->diff(y);
    CHECK(r->__str__() == "Derivative(f(x, y), x, y)");
    r1 = Subs::create(Derivative::create(function_symbol("f", {y, x}), {x}),
                      {{x, add(x, y)}});
    CHECK(r1->__str__() == "Subs(Derivative(f(y, x), x), (x), (x + y))");
}
...
TEST_CASE("test_uexpr_poly(): printing", "[printing]")
{
    ...
    CHECK(p->__str__() == "d*x**(-1)");
    CHECK(not(p->__str__() == "d*x**-1"));

    p = uexpr_poly(x, {{-2, d}, {-1, c}, {0, b}, {1, a}});
    CHECK(p->__str__() == "a*x + b + c*x**(-1) + d*x**(-2)");
}
...
TEST_CASE("test_floats(): printing", "[printing]")
{
    ...
    p = mul(p, x);
    CHECK(p->__str__() == "(0.1 + 0.2*I)*x");

    p = real_double(123);
    p = sub(p, x);
    CHECK(p->__str__() == "123.0 - x");

    p = complex_double(std::complex<double>(1, 2));
    p = add(p, x);
    CHECK(p->__str__() == "1.0 + 2.0*I + x");
    ...
    p = add(p, x);
    CHECK(p->__str__() == "123.0000000000000000000 + x");
...
TEST_CASE("test_functions(): printing", "[printing]")
{
    ...
    CHECK(p->__str__() == "loggamma(x)");

    p = erf(x);
    CHECK(p->__str__() == "erf(x)");

    p = erf(add(x, y));
    CHECK(p->__str__() == "erf(x + y)");

    p = erfc(x);
    CHECK(p->__str__() == "erfc(x)");
}
