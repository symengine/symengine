...
TEST_CASE("basic_to_poly UInt", "[b2poly]")
{
    ...
    basic = add(x, y);
    gen = x;
    CHECK_THROWS_AS(UIntPoly::from_basic(basic, gen), std::runtime_error);
    ...
    // x + y
    basic = add(x, y);
    CHECK_THROWS_AS(UIntPoly::from_basic(basic), std::runtime_error);
...
TEST_CASE("basic_to_poly UExpr", "[b2poly]")
{
    ...
    gen = pow(x, hf);
    poly1 = UExprPoly::from_basic(basic, gen);
