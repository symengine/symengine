...
TEST_CASE("Adding two UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(c->__str__() == "(a + b)*x**2 + 5*x + 3");

    RCP<const UExprPoly> d = uexpr_poly(x, {{0, Expression(2)}});
    CHECK(add_upoly(a, *d)->__str__() == "a*x**2 + 2*x + 3");
    CHECK(add_upoly(*d, a)->__str__() == "a*x**2 + 2*x + 3");
...
TEST_CASE("Negative of a UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(b->__str__() == "-c*x**2 - a*x - 1");

    RCP<const UExprPoly> c = uexpr_poly(x, map_int_Expr{});
    CHECK(neg_upoly(*c)->__str__() == "0");

    c = uexpr_poly(x, {{0, Expression(2)}});
    CHECK(neg_upoly(*c)->__str__() == "-2");
}
...
TEST_CASE("Subtracting two UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(c->__str__() == "(-1 + a)*x**2 + (-2 + b)*x + 1");

    RCP<const UExprPoly> d = uexpr_poly(x, {{0, Expression(2)}});
    CHECK(sub_upoly(a, *d)->__str__() == "x**2 + 2*x - 1");
    CHECK(sub_upoly(*d, a)->__str__() == "-x**2 - 2*x + 1");
...
TEST_CASE("Multiplication of two UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(d->__str__() == "-a**2*x**4 + (-2*a - a*b)*x**3 + (-2*a - "
                          "2*b)*x**2 + (-2 - b)*x - 1");

    RCP<const UExprPoly> f = uexpr_poly(x, {{0, Expression(2)}});
    CHECK(mul_upoly(*a, *f)->__str__() == "2*a*x**2 + 2*b*x + 2");
    CHECK(mul_upoly(*f, *a)->__str__() == "2*a*x**2 + 2*b*x + 2");

    f = uexpr_poly(y, {{0, 2}, {1, 4}});
    CHECK_THROWS_AS(mul_upoly(*a, *f), SymEngineException);

    f = uexpr_poly(x, map_int_Expr{});
    CHECK(mul_upoly(*a, *f)->__str__() == "0");

    a = uexpr_poly(x, {{-2, 5}, {-1, 3}, {0, 1}, {1, 2}});

    c = mul_upoly(*a, *b);
    CHECK(c->__str__() == "-2*a*x**3 + (-4 - a)*x**2 + (-4 - 3*a)*x + (-7 - "
                          "5*a) - 13*x**(-1) - 5*x**(-2)");
}
...
TEST_CASE("Comparing two UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(P->compare(*Q) == 0);
    CHECK(P->__eq__(*Q) == true);

    P = uexpr_poly(x, {{0, 1}, {1, symbol("a")}, {2, 1}});
    CHECK(P->compare(*Q) == -1);
    CHECK(P->__eq__(*Q) == false);
}
...
TEST_CASE("UExprPoly get_args", "[UExprPoly]")
{
    ...
    CHECK(eq(*a->as_symbolic(),
             *add({one, mul(integer(2), x), pow(x, integer(2))})));
    CHECK(not eq(*a->as_symbolic(),
    ...
    CHECK(eq(*a->as_symbolic(), *add({zero})));
}
...
TEST_CASE("UExprPoly max_coef", "[UExprPoly]")
{
    ...
    CHECK(a->max_coef() == 4);
    CHECK(not(a->max_coef() == 2));
    CHECK(b->max_coef() == d);
    CHECK(not(b->max_coef() == c));
    CHECK(not(b->max_coef() == 2));
}
...
TEST_CASE("Evaluation of UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(a->eval(2).get_basic()->__str__() == "5 + 4*a");

    a = uexpr_poly(x, {{-2, 5}, {-1, 3}, {0, 1}, {1, 2}});
    CHECK(a->eval(2).get_basic()->__str__() == "31/4");
}
...
TEST_CASE("Convert UExprPoly to Symbolic", "[UExprPoly]")
{
    ...
    CHECK(eq(*a->get_poly().get_basic("x"),
             *add(integer(1),
...
TEST_CASE("Derivative of UExprPoly", "[UExprPoly]")
{
    ...
    CHECK(a->diff(x)->__str__() == "2*a*x + 2");
    CHECK(a->diff(y)->__str__() == "0");
    CHECK(b->diff(y)->__str__() == "0");

    a = uexpr_poly(x, {{-2, 5}, {-1, 3}, {0, 1}, {1, 2}, {2, symbol("a")}});
    CHECK(a->diff(x)->__str__() == "2*a*x + 2 - 3*x**(-2) - 10*x**(-3)");

    CHECK(c->diff(x)->__str__() == "0");
...
TEST_CASE("Bool checks specific UExprPoly cases", "[UExprPoly]")
{
    ...
    CHECK((z->is_zero() and not z->is_one() and not z->is_minus_one()
           and z->is_integer() and not z->is_symbol() and not z->is_mul()
           and not z->is_pow()));
    CHECK((not o->is_zero() and o->is_one() and not o->is_minus_one()
           and o->is_integer() and not o->is_symbol() and not o->is_mul()
           and not o->is_pow()));
    CHECK((not mo->is_zero() and not mo->is_one() and mo->is_minus_one()
           and mo->is_integer() and not mo->is_symbol() and not mo->is_mul()
           and not mo->is_pow()));
    CHECK((not i->is_zero() and not i->is_one() and not i->is_minus_one()
           and i->is_integer() and not i->is_symbol() and not i->is_mul()
           and not i->is_pow()));
    CHECK((not s->is_zero() and not s->is_one() and not s->is_minus_one()
           and not s->is_integer() and s->is_symbol() and not s->is_mul()
           and not s->is_pow()));
    CHECK((not m1->is_zero() and not m1->is_one() and not m1->is_minus_one()
           and not m1->is_integer() and not m1->is_symbol() and m1->is_mul()
           and not m1->is_pow()));
    CHECK((not m2->is_zero() and not m2->is_one() and not m2->is_minus_one()
           and not m2->is_integer() and not m2->is_symbol() and m2->is_mul()
           and not m2->is_pow()));
    CHECK((not po->is_zero() and not po->is_one() and not po->is_minus_one()
           and not po->is_integer() and not po->is_symbol() and not po->is_mul()
           and po->is_pow()));
    CHECK((not poly->is_zero() and not poly->is_one()
           and not poly->is_minus_one() and not poly->is_integer()
           and not poly->is_symbol() and not poly->is_mul()
           and not poly->is_pow()));
    CHECK((not neg->is_zero() and not neg->is_one() and not neg->is_minus_one()
           and not neg->is_integer() and not neg->is_symbol()
           and not neg->is_mul() and not neg->is_pow()));
}
...
TEST_CASE("UExprPoly expand", "[UExprPoly][expand]")
{
    ...
    CHECK(b->__str__() == "(a*x**3 + x**2 + x)**3");
    CHECK(c->__str__() == "a**3*x**9 + 3*a**2*x**8 + (2*a + a*(1 + 2*a) + "
