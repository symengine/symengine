...
TEST_CASE("Parsing: integers, basic operations", "[parser]")
{
    ...
    s = "-3-5";
    res = parse(s);
    CHECK(eq(*res, *integer(-8)));

    s = "((3)+(1*0))";
    res = parse(s);
    CHECK(eq(*res, *integer(3)));

    s = "((2))*(1+(2*3))";
    res = parse(s);
    CHECK(eq(*res, *integer(14)));
    ...
    s = "(1*3)*(2+4)*(2)";
    res = parse(s);
    CHECK(eq(*res, *integer(36)));

    s = "(1+3)/(2+4)";
    res = parse(s);
    CHECK(eq(*res, *div(integer(2), integer(3))));

    s = "2*3 + 50*2";
    res = parse(s);
    CHECK(eq(*res, *integer(106)));

    s = "2**(3+2)+ 10";
    res = parse(s);
    CHECK(eq(*res, *integer(42)));

    s = "(5^3)/8 + 12";
    res = parse(s);
    CHECK(eq(*res, *add(div(integer(125), integer(8)), integer(12))));

    s = "3*2+3-5+2/2";
    res = parse(s);
    CHECK(eq(*res, *integer(5)));

    s = "4^2/2+2";
    res = parse(s);
    CHECK(eq(*res, *integer(10)));

    s = "(1+2*(3+1)-5/(2+2))";
    res = parse(s);
    CHECK(eq(*res, *add(integer(9), div(integer(-5), integer(4)))));

    s = "2 + -3";
    res = parse(s);
    CHECK(eq(*res, *integer(-1)));

    s = "10000000000000000000000000";
    res = parse(s);
    CHECK(eq(*res, *pow(integer(10), integer(25))));

    // Make sure that parsing and printing works correctly
    s = "0.123123123e-10";
    res = parse(s);
    CHECK(eq(*res, *parse(res->__str__())));
...
TEST_CASE("Parsing: symbols", "[parser]")
{
    ...
    s = "x + 2*y";
    res = parse(s);
    CHECK(eq(*res, *add(x, mul(integer(2), y))));
    ...
    s = "w1*y";
    res = parse(s);
    CHECK(eq(*res, *mul(w, y)));

    s = "x**(3+w1)-2/y";
    res = parse(s);
    CHECK(eq(*res, *add(pow(x, add(integer(3), w)), div(integer(-2), y))));

    s = "l0ngn4me - w1*y + 2^(x)";
    res = parse(s);
    CHECK(eq(*res, *add(add(l, neg(mul(w, y))), pow(integer(2), x))));

    s = "4*x/8 - (w1*y)";
    res = parse(s);
    CHECK(eq(*res, *add(neg(mul(w, y)), div(x, integer(2)))));

    s = "3*y + (2*y)";
    res = parse(s);
    CHECK(eq(*res, *mul(y, integer(5))));

    s = "3*y/(1+x)";
    res = parse(s);
    CHECK(eq(*res, *div(mul(y, integer(3)), add(x, integer(1)))));
    ...
    s = "x * -y";
    res = parse(s);
    CHECK(eq(*res, *mul(x, mul(y, integer(-1)))));

    s = "x ^ --y";
    res = parse(s);
    CHECK(eq(*res, *pow(x, y)));
    ...
    s = "x**2e-1+3e+2-2e-2";
    res = parse(s);
    CHECK(eq(*res, *add(real_double(299.98), pow(x, real_double(0.2)))));
}
...
TEST_CASE("Parsing: functions", "[parser]")
{
    ...
    s = "asin(-1)";
    res = parse(s);
    CHECK(eq(*res, *neg(div(pi, integer(2)))));

    s = "asin(sin(x))";
    res = parse(s);
    CHECK(eq(*res, *asin(sin(x))));

    s = "beta(x, y)";
    res = parse(s);
    CHECK(eq(*res, *beta(x, y)));

    s = "erf(erf(x*y)) + y";
    res = parse(s);
    CHECK(eq(*res, *add(erf(erf(mul(x, y))), y)));

    s = "erfc(sin(x))+erfc(x*y)";
    res = parse(s);
    CHECK(eq(*res, *add(erfc(mul(x, y)), erfc(sin(x)))));

    s = "beta(sin(x+3), gamma(2^y+sin(y)))";
    res = parse(s);
    CHECK(eq(*res, *beta(sin(add(x, integer(3))),
    ...
    res = parse(s);
    CHECK(eq(*res, *add(integer(2),
                        add(zeta(integer(2), x), zeta(log(integer(3)))))));

    s = "sin(asin(x)) + y";
    res = parse(s);
    CHECK(eq(*res, *add(x, y)));

    s = "log(x, gamma(y))*sin(3)";
    res = parse(s);
    CHECK(eq(*res, *mul(log(x, gamma(y)), sin(integer(3)))));

    s = "loggamma(x)*gamma(y)";
    res = parse(s);
    CHECK(eq(*res, *mul(loggamma(x), gamma(y))));

    s = "loggamma(x)+loggamma(x)";
    res = parse(s);
    CHECK(eq(*res, *mul(integer(2), loggamma(x))));

    s = "max(x, x, y)";
    res = parse(s);
    CHECK(eq(*res, *max({x, y})));
    ...
    res = parse(s);
    CHECK(eq(*res, *sin(max({log(x, y), min({x, y})}))));
}
...
TEST_CASE("Parsing: constants", "[parser]")
{
    ...
    res = parse(s);
    CHECK(eq(*res, *mul(integer(3), x)));

    s = "(3+4*I)/(5+cos(pi/2)*I)";
    res = parse(s);
    CHECK(eq(*res, *div(Complex::from_two_nums(*integer(3), *integer(4)),
                        integer(5))));

    s = "(2*I +6*I)*3*I + 4*I";
    res = parse(s);
    CHECK(eq(*res, *Complex::from_two_nums(*integer(-24), *integer(4))));
    ...
    s = "1/oo + 2";
    res = parse(s);
    CHECK(eq(*res, *integer(2)));

    s = "zoo";
    res = parse(s);
    CHECK(eq(*res, *ComplexInf));
}
...
TEST_CASE("Parsing: function_symbols", "[parser]")
{
    ...
    s = "f(x)";
    res = parse(s);
    CHECK(eq(*res, *function_symbol("f", x)));

    s = "my_func(x, wt) + sin(f(y))";
    res = parse(s);
    CHECK(eq(*res, *add(function_symbol("my_func", {x, z}),
                        sin(function_symbol("f", y)))));

    s = "func(x, y, wt) + f(sin(x))";
    res = parse(s);
    CHECK(eq(*res, *add(function_symbol("func", {x, y, z}),
                        function_symbol("f", sin(x)))));

    s = "f(g(2^x))";
    res = parse(s);
    CHECK(eq(*res,
             *function_symbol("f", function_symbol("g", pow(integer(2), x)))));
}
...
TEST_CASE("Parsing: doubles", "[parser]")
{
    ...
    s = "1.324";
    res = parse(s);
    CHECK(eq(*res, *real_double(1.324)));

    s = "0.0324*x + 2*3";
    res = parse(s);
    CHECK(eq(*res, *add(mul(real_double(0.0324), x), integer(6))));
