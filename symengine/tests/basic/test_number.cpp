...
TEST_CASE("RealMPFR: arithmetic", "[number]")
{
    ...
    CHECK(not r1->is_one());
    CHECK(not r1->is_minus_one());
    CHECK(not r1->is_complex());

    CHECK(eq(*r2, *addnum(r1, r1)));
    REQUIRE(is_a<RealMPFR>(*addnum(r1, rat1)));
    CHECK(eq(*r2, *addnum(r1, rd1)));
    CHECK(eq(*r1, *subnum(r1, integer(0))));
    REQUIRE(is_a<RealMPFR>(*subnum(r2, rat1)));
    CHECK(eq(*r1, *subnum(r2, rd1)));
    REQUIRE(is_a<RealMPFR>(*subnum(rat1, r2)));
    CHECK(eq(*neg(r1), *subnum(rd1, r2)));
    CHECK(eq(*r2, *mulnum(r1, i2)));
    CHECK(eq(*r2, *mulnum(i2, r1)));
    CHECK(eq(*r3, *mulnum(r1, rd1)));
    REQUIRE(is_a<RealMPFR>(*mulnum(rat1, r1)));
    CHECK(eq(*r3, *mulnum(r1, r1)));
    CHECK(eq(*r1, *divnum(r2, i2)));
    CHECK(eq(*r1, *divnum(r3, rd1)));
    REQUIRE(is_a<RealMPFR>(*divnum(r3, rat1)));
    CHECK(eq(*r1, *divnum(r3, r1)));
    REQUIRE(is_a<RealMPFR>(*divnum(mulnum(rat1, rat1), r1)));
    CHECK(eq(*r1, *divnum(mulnum(rd1, rd1), r1)));
    CHECK(eq(*divnum(i1, r1), *divnum(i2, r2)));
    CHECK(eq(*r1, *subnum(r2, r1)));
    CHECK(eq(*r1, *subnum(integer(20), r1)));
    CHECK(eq(*r1, *mulnum(r2, half)));
    CHECK(eq(*r3, *pownum(r1, i2)));
    CHECK(eq(*r4, *pownum(i2, r1)));
    CHECK(eq(*r1, *pownum(r3, half)));
    CHECK(eq(*r3, *pownum(r1, real_double(2.0))));
    CHECK(eq(*r3, *pownum(r1, divnum(r2, r1))));
    CHECK(eq(*r4, *pownum(real_double(2.0), r1)));
    REQUIRE(is_a<RealMPFR>(*pownum(rat1, r1)));
    CHECK(eq(*divnum(i1, r4), *half->pow(*r1)));
    ...
    CHECK(r1->compare(*r2) == 1); // TO-DO is this is a bug or what ?
...
TEST_CASE("ComplexMPC: arithmetic", "[number]")
{
    ...
    CHECK(eq(*r2, *addnum(c1, r1)));
    CHECK(eq(*r2, *addnum(r1, r1)));
    CHECK(eq(*r2, *addnum(r5, i10)));
    REQUIRE(is_a<ComplexMPC>(*addnum(r5, rat1)));
    CHECK(eq(*r2, *addnum(r5, rd1)));
    CHECK(eq(*r2, *addnum(r1, cd1)));
    CHECK(eq(*r1, *subnum(r2, r1)));
    CHECK(eq(*r1, *subnum(mulnum(c1, i2), r1)));
    REQUIRE(is_a<ComplexMPC>(*subnum(r2, rat1)));
    CHECK(eq(*r5, *subnum(r2, rd1)));
    CHECK(eq(*r5, *subnum(r2, i10)));
    CHECK(eq(*r1, *subnum(r2, c1)));
    CHECK(eq(*r1, *subnum(r2, cd1)));
    REQUIRE(is_a<ComplexMPC>(*subnum(rat1, r2)));
    CHECK(eq(*neg(r5), *subnum(rd1, r2)));
    CHECK(eq(*neg(r5), *subnum(i10, r2)));
    CHECK(eq(*neg(r1), *subnum(c1, r2)));
    CHECK(eq(*neg(r1), *subnum(cd1, r2)));
    CHECK(eq(*r2, *mulnum(r1, i2)));
    CHECK(eq(*r2, *mulnum(i2, r1)));
    CHECK(eq(*mulnum(cd1, r1), *mulnum(c1, r1)));
    CHECK(eq(*r3, *mulnum(r1, r1)));
    CHECK(eq(*mulnum(r1, i10), *mulnum(r1, rd1)));
    CHECK(eq(*r1, *mulnum(r2, half)));
    CHECK(eq(*r1, *divnum(r2, i2)));
    CHECK(eq(*r2, *divnum(r1, half)));
    CHECK(eq(*divnum(r2, c1), *divnum(r2, cd1)));
    CHECK(eq(*divnum(i1, r1), *divnum(i2, r2)));
    CHECK(eq(*r1, *divnum(r3, r1)));
    CHECK(eq(*r1, *divnum(r2, real_double(2.0))));
    CHECK(eq(*divnum(one, r1), *divnum(i2, r2)));
    CHECK(eq(*divnum(one, r2), *divnum(half, r1)));
    CHECK(eq(*divnum(c1, r2), *divnum(cd1, r2)));
    CHECK(eq(*divnum(i1, r1), *divnum(i2, r2)));
    CHECK(eq(*divnum(one, r1), *divnum(real_double(2.0), r2)));
    CHECK(eq(*r3, *pownum(r1, i2)));
    CHECK(eq(*r3, *pownum(r1, real_double(2.0))));
    REQUIRE(is_a<ComplexMPC>(*pownum(r1, cd1)));
    CHECK(eq(*pownum(r1, r1), *pownum(r1, cd1)));
    CHECK(eq(*pownum(r1, c1), *pownum(r1, cd1)));
    CHECK(eq(*r3, *pownum(r1, real_double(2.0))));
    CHECK(eq(*r1, *pownum(r3, half)));
    CHECK(eq(*pownum(c1, r1), *pownum(cd1, r1)));
    CHECK(eq(*r1, *pownum(r3, real_double(0.5))));
    CHECK(eq(*r4, *pownum(i2, r1)));
    CHECK(eq(*r4, *pownum(real_double(2.0), r1)));
    CHECK(eq(*divnum(i1, r4), *half->pow(*r1)));
    ...
    CHECK(r6->__hash__() == r2->__hash__());
    CHECK(r6->compare(*r2) == 0);
    CHECK(r6->compare(*r3) == -1);
    CHECK(r3->compare(*r6) == 1);
    CHECK(r2->compare(*r5) == 1);

    mpc_class temp(101);
    mpc_set_ui_ui(temp.get_mpc_t(), 20, 14, MPFR_RNDN);
    CHECK(complex_mpc(std::move(temp))->compare(*r6) == 1);
    ...
    CHECK(eq(*r2, *addnum(r5, r6)));
    CHECK(eq(*r5, *subnum(r2, r6)));
    CHECK(eq(*neg(r5), *subnum(r6, r2)));
    CHECK(eq(*mulnum(r2, i10), *mulnum(r6, r2)));
    REQUIRE(is_a<ComplexMPC>(*divnum(r3, r6)));
    REQUIRE(is_a<ComplexMPC>(*divnum(r6, r3)));
    CHECK(eq(*r3, *pownum(r1, divnum(r6, integer(5)))));
    CHECK(eq(*r1, *pownum(r3, divnum(r6, integer(20)))));
    CHECK(eq(*r4, *pownum(divnum(r6, integer(5)), r1)));
#endif // HAVE_SYMENGINE_MPFR
#endif // HAVE_SYMENGINE_MPC
}
...
TEST_CASE("Test is_exact", "[number]")
{
    ...
    CHECK(n1->is_exact());
    CHECK(n1->is_zero());
    CHECK(n1->is_exact_zero());
    CHECK(not n1->is_negative());
    CHECK(not n1->is_positive());
    CHECK(not n1->is_complex());

    n1 = integer(-1);
    CHECK(n1->is_exact());
    CHECK(not n1->is_zero());
    CHECK(not n1->is_exact_zero());
    CHECK(n1->is_negative());
    CHECK(not n1->is_positive());
    CHECK(not n1->is_complex());
...
TEST_CASE("Test NumberWrapper", "[number]")
{
    ...
    CHECK(eq(*integer(5)->add(*n), *make_rcp<Long>(15)));
    CHECK(eq(*integer(5)->mul(*n), *make_rcp<Long>(50)));

    RCP<const Basic> r = add(sqrt(integer(2)), n);
    double d1 = eval_double(*r);
    CHECK(std::abs(d1 - 11.4142135623730951) < 1e-12);
    CHECK(n->__str__() == "10");

#ifdef HAVE_SYMENGINE_MPFR
    mpfr_class aa(100);
    eval_mpfr(aa.get_mpfr_t(), *r, MPFR_RNDN);
    CHECK(mpfr_cmp_d(aa.get_mpfr_t(), 11.41421356237309) == 1);
    CHECK(mpfr_cmp_d(aa.get_mpfr_t(), 11.41421356237310) == -1);
#ifdef HAVE_SYMENGINE_MPC
    mpc_class a(100);
