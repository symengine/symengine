...
TEST_CASE("Integer: Basic", "[basic]")
{
    ...
    CHECK_THROWS_AS(divnum(i, zero), SymEngineException);
}
...
TEST_CASE("Rational: Basic", "[basic]")
{
    ...
    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = zero;
    CHECK_THROWS_AS(divnum(r1, r2), SymEngineException);
...
TEST_CASE("Mul: Basic", "[basic]")
{
    ...
    CHECK_THROWS_AS(div(integer(1), zero), SymEngineException);
...
TEST_CASE("Complex: Basic", "[basic]")
{
    ...
    CHECK_THROWS_AS(divnum(c1, integer(0)), SymEngineException);

    r3 = Rational::from_two_ints(*integer(0), *integer(1));
    CHECK_THROWS_AS(divnum(c1, r3), SymEngineException);

    c2 = Complex::from_two_nums(*r3, *r3);
    CHECK_THROWS_AS(divnum(c1, c2), SymEngineException);
}
