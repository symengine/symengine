#include "catch.hpp"
#include <symengine/visitor.h>
#include <symengine/eval_double.h>
#include <symengine/derivative.h>
#include <symengine/symengine_exception.h>
#include <cstring>

using SymEngine::Basic;
using SymEngine::Add;
using SymEngine::Mul;
using SymEngine::Symbol;
using SymEngine::symbol;
using SymEngine::umap_basic_num;
using SymEngine::map_basic_num;
using SymEngine::map_basic_basic;
using SymEngine::umap_basic_basic;
using SymEngine::map_uint_mpz;
using SymEngine::unified_compare;
using SymEngine::map_int_Expr;
using SymEngine::multiset_basic;
using SymEngine::vec_basic;
using SymEngine::set_basic;
using SymEngine::Integer;
using SymEngine::integer;
using SymEngine::Rational;
using SymEngine::real_double;
using SymEngine::one;
using SymEngine::zero;
using SymEngine::Number;
using SymEngine::pow;
using SymEngine::RCP;
using SymEngine::make_rcp;
using SymEngine::print_stack_on_segfault;
using SymEngine::Complex;
using SymEngine::complex_double;
using SymEngine::has_symbol;
using SymEngine::coeff;
using SymEngine::is_a;
using SymEngine::rcp_static_cast;
using SymEngine::set_basic;
using SymEngine::free_symbols;
using SymEngine::function_symbol;
using SymEngine::rational_class;
using SymEngine::pi;
using SymEngine::diff;
using SymEngine::sdiff;
using SymEngine::down_cast;
using SymEngine::NotImplementedError;
using SymEngine::ComplexInf;
using SymEngine::Nan;
using SymEngine::EulerGamma;

using namespace SymEngine::literals;

TEST_CASE("Test version", "[basic]")
{
    CHECK(std::strcmp(SymEngine::get_version(), SYMENGINE_VERSION) == 0);
}

TEST_CASE("Symbol hash: Basic", "[basic]")
{
    RCP<const Symbol> x = symbol("x");
    RCP<const Symbol> x2 = symbol("x");
    RCP<const Symbol> y = symbol("y");

    CHECK(x->__eq__(*x));
    CHECK(x->__eq__(*x2));
    CHECK(not(x->__eq__(*y)));
    CHECK(x->__neq__(*y));

    std::hash<Basic> hash_fn;
    // Hashes of x and x2 must be the same:
    CHECK(hash_fn(*x) == hash_fn(*x2));

    // This checks that the hash of the Symbols are ordered:
    CHECK(hash_fn(*x) < hash_fn(*y));
}

TEST_CASE("Symbol dict: Basic", "[basic]")
{
    umap_basic_num ubn;
    map_basic_num mbn;
    umap_basic_basic ubb;
    map_basic_basic mbb;
    vec_basic vb;
    set_basic sb;
    RCP<const Basic> x = symbol("x");
    RCP<const Basic> x2 = symbol("x");
    RCP<const Basic> y = symbol("y");
    RCP<const Number> i2 = integer(2);
    RCP<const Number> i3 = integer(3);
    bool p = (x != x2);
    CHECK(p);           // The instances are different...
    CHECK(eq(*x, *x2)); // ...but equal in the SymPy sense

    std::stringstream buffer;
    buffer << ubn;
    CHECK(buffer.str() == "{}");
    insert(ubn, x, i2);
    buffer.str("");
    buffer << ubn;
    CHECK(buffer.str() == "{x: 2}");
    insert(ubn, y, i3);
    buffer.str("");
    buffer << mbn;
    CHECK(buffer.str() == "{}");
    insert(mbn, x, i2);
    insert(mbn, y, i3);
    buffer.str("");
    buffer << mbb;
    CHECK(buffer.str() == "{}");
    insert(mbb, x, i2);
    insert(mbb, y, i3);
    buffer.str("");
    buffer << ubb;
    CHECK(buffer.str() == "{}");
    insert(ubb, x, i2);
    insert(ubb, y, i3);
    buffer.str("");
    buffer << vb;
    CHECK(buffer.str() == "{}");
    vb.push_back(x);
    vb.push_back(i3);
    buffer.str("");
    buffer << vb;
    CHECK(buffer.str() == "{x, 3}");
    CHECK(unified_eq(vb, {x, i3}));
    CHECK(unified_compare(vb, {x, i3}) == 0);
    CHECK(not unified_eq(vb, {i3, x}));
    CHECK(vec_basic_eq_perm(vb, {i3, x}));
    CHECK(not unified_eq(vb, {i3}));
    CHECK(not vec_basic_eq_perm(vb, {i3}));
    CHECK(unified_compare(vb, {i3}) == 1);
    buffer.str("");
    buffer << sb;
    CHECK(buffer.str() == "{}");
    sb.insert(i2);
    sb.insert(y);

    auto check_map_str = [](std::string to_chk, std::vector<std::string> key,
                            std::vector<std::string> val) {
        if (key.size() != val.size())
            return false;
        for (unsigned i = 0; i < key.size(); i++) {
            if (to_chk.find(key[i] + std::string(": " + val[i]))
                == std::string::npos)
                return false;
        }
        return true;
    };

    buffer.str("");
    buffer << ubn;
    CHECK(check_map_str(buffer.str(), {"x", "y"}, {"2", "3"}));
    buffer.str("");
    buffer << mbn;
    CHECK(check_map_str(buffer.str(), {"x", "y"}, {"2", "3"}));
    buffer.str("");
    buffer << mbb;
    CHECK(check_map_str(buffer.str(), {"x", "y"}, {"2", "3"}));
    buffer.str("");
    buffer << ubb;
    CHECK(check_map_str(buffer.str(), {"x", "y"}, {"2", "3"}));
    buffer.str("");
    buffer << ubb;
    CHECK(check_map_str(buffer.str(), {"x", "y"}, {"2", "3"}));
    buffer.str("");
    buffer << vb;
    bool check_vec_str;
    check_vec_str = buffer.str() == "{x, 3}";
    CHECK(check_vec_str);
    buffer.str("");
    buffer << sb;
    check_vec_str = buffer.str() == "{2, y}";
    CHECK(check_vec_str);

    map_uint_mpz a = {{0, 1_z}, {1, 2_z}, {2, 1_z}};
    map_uint_mpz b = {{0, 1_z}, {2, 1_z}, {1, 2_z}};
    CHECK(unified_compare(a, b) == 0);
    b = {{0, 1_z}, {2, 1_z}};
    CHECK(unified_compare(a, b) == 1);
    b = {{0, 1_z}, {3, 1_z}, {1, 2_z}};
    CHECK(unified_compare(a, b) == -1);
    b = {{0, 1_z}, {3, 1_z}, {1, 2_z}};
    CHECK(unified_compare(a, b) == -1);
    b = {{0, 1_z}, {1, 1_z}, {2, 3_z}};
    CHECK(unified_compare(a, b) == 1);

    map_int_Expr adict = {{0, 1}, {1, 2}, {2, x}};
    map_int_Expr bdict = {{0, 1}, {1, 2}, {2, x}};
    CHECK(unified_compare(adict, bdict) == 0);
    bdict = {{0, 1}, {1, 1}, {2, x}};
    CHECK(unified_compare(adict, bdict) != 0);
    CHECK(unified_compare(adict, bdict) == -unified_compare(bdict, adict));
    adict = {{0, 1}, {1, 1}, {3, x}};
    CHECK(unified_compare(adict, bdict) != 0);
    CHECK(unified_compare(adict, bdict) == -unified_compare(bdict, adict));
    bdict = {{0, 1}, {1, 3}};
    CHECK(unified_compare(adict, bdict) == 1);

    multiset_basic msba, msbb;
    msba.insert(x);
    msba.insert(y);
    msba.insert(i2);
    msbb.insert(y);
    msbb.insert(i2);
    CHECK(not unified_eq(msba, msbb));
    CHECK(unified_compare(msba, msbb) == 1);
    msbb.insert(x);
    CHECK(unified_eq(msba, msbb));
    CHECK(unified_compare(msba, msbb) == 0);
    msbb.insert(i3);
    CHECK(not unified_eq(msba, msbb));
    CHECK(unified_compare(msba, msbb) == -1);
    CHECK(not unified_eq(msba, {x, y, i3}));
    CHECK(unified_compare(msba, {x, y, i3}) == -1);
}

TEST_CASE("Add: basic", "[basic]")
{
    umap_basic_num m, m2;
    RCP<const Basic> x = symbol("x");
    RCP<const Basic> y = symbol("y");
    insert(m, x, integer(2));
    insert(m, y, integer(3));

    m2 = m;
    RCP<const Add> a = make_rcp<const Add>(zero, std::move(m2));
    insert(m, x, integer(-2));
    RCP<const Add> b = make_rcp<const Add>(zero, std::move(m));
    std::cout << *a << std::endl;
    std::cout << *b << std::endl;

    RCP<const Basic> r = add(add(x, y), add(y, x));
    std::cout << *r << std::endl;

    r = add(x, x);
    std::cout << *r << std::endl;

    r = add(add(x, x), y);
    std::cout << *r << std::endl;
    std::cout << "----------------------" << std::endl;

    CHECK(vec_basic_eq_perm(r->get_args(), {mul(integer(2), x), y}));
    CHECK(not vec_basic_eq_perm(r->get_args(), {mul(integer(3), x), y}));

    RCP<const Add> ar = rcp_static_cast<const Add>(r);
    CHECK(eq(*ar->get_coef(), *zero));
    const umap_basic_num &addmap = ar->get_dict();
    auto search = addmap.find(x);
    CHECK(search != addmap.end());
    CHECK(eq(*search->second, *integer(2)));
    search = addmap.find(y);
    CHECK(search != addmap.end());
    CHECK(eq(*search->second, *integer(1)));

    RCP<const Basic> term1, term2;
    RCP<const Add> a1 = rcp_static_cast<const Add>(add(r, r));
    a1->as_two_terms(outArg(term1), outArg(term2));
    RCP<const Add> a2 = rcp_static_cast<const Add>(add(term1, term2));
    CHECK(eq(*a1, *a2));

    r = add(mul(integer(5), x), integer(5));
    ar = rcp_static_cast<const Add>(r);
    CHECK(eq(*ar->get_coef(), *integer(5)));
    CHECK(vec_basic_eq_perm(r->get_args(), {mul(integer(5), x), integer(5)}));

    r = add(add(mul(mul(integer(2), x), y), integer(5)), pow(x, integer(2)));
    CHECK(vec_basic_eq_perm(
        r->get_args(),
        {integer(5), mul(mul(integer(2), x), y), pow(x, integer(2))}));
    std::cout << *r << std::endl;
}

TEST_CASE("Integer: Basic", "[basic]")
{
    RCP<const Integer> i = integer(5);
    RCP<const Integer> j = integer(6);
    RCP<const Basic> r;
    std::cout << *i << std::endl;
    std::cout << *j << std::endl;

    RCP<const Number> k = addnum(i, j);
    std::cout << *k << std::endl;
    CHECK(eq(*k, *integer(11)));
    CHECK(neq(*k, *integer(12)));

    k = subnum(i, j);
    std::cout << *k << std::endl;
    CHECK(eq(*k, *integer(-1)));
    CHECK(neq(*k, *integer(12)));

    k = mulnum(i, j);
    std::cout << *k << std::endl;
    CHECK(eq(*k, *integer(30)));
    CHECK(neq(*k, *integer(12)));

    k = divnum(i, j);
    CHECK(eq(*k, *Rational::from_two_ints(*integer(5), *integer(6))));
    std::cout << *k << std::endl;

    k = divnum(i, zero);
    CHECK(eq(*k, *ComplexInf));

    k = divnum(zero, i);
    CHECK(eq(*k, *zero));

    k = divnum(zero, zero);
    CHECK(eq(*k, *Nan));

    k = pownum(i, j);
    CHECK(eq(*k, *integer(15625)));
    std::cout << *k << std::endl;

    k = pownum(i, j->neg());
    CHECK(eq(*k, *Rational::from_two_ints(*integer(1), *integer(15625))));
    std::cout << *k << std::endl;

    k = i->neg();
    std::cout << *k << std::endl;
    CHECK(eq(*k, *integer(-5)));
    CHECK(neq(*k, *integer(12)));

    REQUIRE(not i->is_complex());

    i = integer(0);
    j = integer(0);
    r = i->div(*j);
    CHECK(eq(*r, *Nan));
}

TEST_CASE("Rational: Basic", "[basic]")
{
    RCP<const Number> r1, r2, r3;
    RCP<const Rational> r;
    rational_class a, b;

    r1 = Rational::from_two_ints(*integer(5), *integer(6));
    std::cout << *r1 << std::endl;
    CHECK(eq(*r1, *Rational::from_two_ints(*integer(5), *integer(6))));
    CHECK(neq(*r1, *Rational::from_two_ints(*integer(5), *integer(7))));

    r1 = Rational::from_two_ints(*integer(2), *integer(4));
    r2 = Rational::from_two_ints(*integer(1), *integer(2));
    CHECK(eq(*r1, *r2));

    r1 = Rational::from_two_ints(*integer(-2), *integer(3));
    r2 = Rational::from_two_ints(*integer(2), *integer(-3));
    CHECK(eq(*r1, *r2));

    r1 = Rational::from_two_ints(*integer(4), *integer(2));
    r2 = integer(2);
    CHECK(eq(*r1, *r2));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = Rational::from_two_ints(*integer(5), *integer(7));
    r3 = Rational::from_two_ints(*integer(10), *integer(21));
    CHECK(eq(*mulnum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = Rational::from_two_ints(*integer(1), *integer(2));
    r3 = Rational::from_two_ints(*integer(1), *integer(3));
    CHECK(eq(*mulnum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = Rational::from_two_ints(*integer(9), *integer(2));
    r3 = integer(3);
    CHECK(eq(*mulnum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(1), *integer(2));
    r2 = integer(1);
    CHECK(eq(*addnum(r1, r1), *r2));

    r1 = Rational::from_two_ints(*integer(1), *integer(2));
    r2 = Rational::from_two_ints(*integer(1), *integer(3));
    r3 = Rational::from_two_ints(*integer(1), *integer(6));
    CHECK(eq(*subnum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(1), *integer(6));
    r2 = Rational::from_two_ints(*integer(1), *integer(3));
    r3 = Rational::from_two_ints(*integer(1), *integer(2));
    CHECK(eq(*divnum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(2);
    r3 = Rational::from_two_ints(*integer(4), *integer(9));
    CHECK(eq(*pownum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(-2);
    r3 = Rational::from_two_ints(*integer(9), *integer(4));
    CHECK(eq(*pownum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(3);
    r3 = Rational::from_two_ints(*integer(8), *integer(27));
    CHECK(eq(*pownum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(-3);
    r3 = Rational::from_two_ints(*integer(27), *integer(8));
    CHECK(eq(*pownum(r1, r2), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(3);
    r3 = integer(2);
    CHECK(eq(*mulnum(r1, r2), *r3));
    CHECK(eq(*mulnum(r2, r1), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(3);
    r3 = Rational::from_two_ints(*integer(11), *integer(3));
    CHECK(eq(*addnum(r1, r2), *r3));
    CHECK(eq(*addnum(r2, r1), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(3);
    r3 = Rational::from_two_ints(*integer(-7), *integer(3));
    CHECK(eq(*subnum(r1, r2), *r3));
    r3 = Rational::from_two_ints(*integer(7), *integer(3));
    CHECK(eq(*subnum(r2, r1), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = integer(3);
    r3 = Rational::from_two_ints(*integer(2), *integer(9));
    CHECK(eq(*divnum(r1, r2), *r3));
    r3 = Rational::from_two_ints(*integer(9), *integer(2));
    CHECK(eq(*divnum(r2, r1), *r3));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = zero;
    CHECK(eq(*divnum(r1, r2), *ComplexInf));

    r1 = Rational::from_two_ints(*integer(2), *zero);
    CHECK(eq(*r1, *ComplexInf));

    r1 = Rational::from_two_ints(2, 0);
    CHECK(eq(*r1, *ComplexInf));

    r1 = Rational::from_two_ints(*zero, *zero);
    CHECK(eq(*r1, *Nan));
    r1 = Rational::from_two_ints(0, 0);
    CHECK(eq(*r1, *Nan));

    r1 = Rational::from_two_ints(*integer(3), *integer(5));
    CHECK(is_a<Rational>(*r1));
    r = rcp_static_cast<const Rational>(r1);
    a = rational_class(3, 5);
    b = r->as_rational_class();
    CHECK(a == b);

    r1 = Rational::from_two_ints(*integer(0), *integer(0));
    r2 = r1->div(*integer(0));
    CHECK(eq(*r2, *Nan));
    r1 = Rational::from_two_ints(*integer(0), *integer(0));
    r2 = r1->div(*Rational::from_two_ints(*integer(0), *integer(0)));
    CHECK(eq(*r2, *Nan));
    r2 = integer(0)->div(*r1);
    CHECK(eq(*r2, *Nan));
}

TEST_CASE("Mul: Basic", "[basic]")
{
    map_basic_basic m, m2;
    RCP<const Basic> x = symbol("x");
    RCP<const Basic> y = symbol("y");
    insert(m, x, integer(2));
    insert(m, y, integer(3));

    m2 = m;
    RCP<const Mul> a = make_rcp<const Mul>(one, std::move(m2));
    insert(m, x, integer(-2));
    RCP<const Mul> b = make_rcp<const Mul>(one, std::move(m));
    std::cout << *a << std::endl;
    std::cout << *b << std::endl;

    RCP<const Basic> r = mul(mul(x, y), mul(y, x));
    std::cout << *r << std::endl;

    CHECK(vec_basic_eq_perm(r->get_args(),
                            {pow(x, integer(2)), pow(y, integer(2))}));

    r = mul(mul(pow(x, integer(3)), integer(2)), y);
    CHECK(
        vec_basic_eq_perm(r->get_args(), {integer(2), pow(x, integer(3)), y}));

    r = add(x, x);
    CHECK(vec_basic_eq_perm(r->get_args(), {x, integer(2)}));

    r = sub(x, x);
    CHECK(unified_eq(r->get_args(), {}));

    r = mul(x, x);
    CHECK(unified_eq(r->get_args(), {x, integer(2)}));

    r = div(x, x);
    CHECK(unified_eq(r->get_args(), {}));

    CHECK(eq(*div(integer(1), zero), *ComplexInf));

    r = mul(mul(mul(x, y), mul(x, integer(2))), integer(3));
    RCP<const Mul> mr = rcp_static_cast<const Mul>(r);
    CHECK(eq(*mr->get_coef(), *integer(6)));
    const map_basic_basic &mulmap = mr->get_dict();
    auto search = mulmap.find(x);
    CHECK(search != mulmap.end());
    CHECK(eq(*search->second, *integer(2)));
    search = mulmap.find(y);
    CHECK(search != mulmap.end());
    CHECK(eq(*search->second, *integer(1)));
}

TEST_CASE("Diff: Basic", "[basic]")
{
    RCP<const Basic> r1, r2;
    RCP<const Symbol> x = symbol("x");
    RCP<const Symbol> y = symbol("y");
    RCP<const Basic> i2 = integer(2);
    RCP<const Basic> i3 = integer(3);
    RCP<const Basic> i5 = integer(5);
    RCP<const Basic> i10 = integer(10);
    r1 = integer(5);
    r2 = r1->diff(x);
    CHECK(eq(*r2, *zero));

    r1 = Rational::from_two_ints(*integer(2), *integer(3));
    r2 = r1->diff(x);
    CHECK(eq(*r2, *zero));

    r1 = pow(x, i3)->diff(x);
    r2 = mul(i3, pow(x, i2));
    CHECK(eq(*r1, *r2));

    r1 = pow(add(x, y), i2)->diff(x);
    r2 = mul(i2, add(x, y));
    CHECK(eq(*r1, *r2));

    r1 = add(add(i2, mul(i3, x)), mul(i5, pow(x, i2)));
    CHECK(eq(*r1->diff(x), *add(i3, mul(i10, x))));
    CHECK(eq(*r1->diff(x)->diff(x), *i10));

    r1 = add(mul(mul(pow(x, y), pow(y, x)), i2), one)->diff(x);
    r2 = add(mul(i2, mul(pow(x, y), mul(pow(y, x), log(y)))),
             mul(i2, mul(pow(x, y), mul(pow(y, x), div(y, x)))));
    CHECK(eq(*r1, *r2));

    r1 = sdiff(add(pow(x, i2), x), pow(x, i2));
    r2 = one;
    CHECK(eq(*r1, *r2));

    r1 = sdiff(add(pow(x, i2), x), x);
    r2 = diff(add(pow(x, i2), x), x);
    CHECK(eq(*r1, *r2));
}

TEST_CASE("compare: Basic", "[basic]")
{
    RCP<const Basic> r1, r2;
    RCP<const Symbol> x = symbol("x");
    RCP<const Symbol> y = symbol("y");
    RCP<const Symbol> z = symbol("z");
    RCP<const Basic> i2 = integer(2);
    RCP<const Basic> im2 = integer(-2);
    RCP<const Basic> i3 = integer(3);
    CHECK(x->compare(*x) == 0);
    CHECK(x->compare(*y) == -1);
    CHECK(x->compare(*z) == -1);
    CHECK(y->compare(*x) == 1);
    CHECK(y->compare(*z) == -1);
    CHECK(z->compare(*x) == 1);
    CHECK(z->compare(*y) == 1);

    CHECK(i2->compare(*i2) == 0);
    CHECK(i2->compare(*i3) == -1);
    CHECK(i3->compare(*i2) == 1);

    r1 = mul(x, y);
    r2 = mul(x, y);
    CHECK(r1->compare(*r2) == 0);
    CHECK(r2->compare(*r1) == 0);

    r1 = mul(x, y);
    r2 = mul(x, z);
    //    CHECK(r1->compare(*r2) == -1);
    //    CHECK(r2->compare(*r1) == 1);

    r1 = mul(y, x);
    r2 = mul(x, z);
    //    CHECK(r1->compare(*r2) == -1);
    //    CHECK(r2->compare(*r1) == 1);

    r1 = mul(y, x);
    r2 = mul(x, z);
    //    CHECK(r1->compare(*r2) == -1);
    //    CHECK(r2->compare(*r1) == 1);

    r1 = mul(mul(y, x), z);
    r2 = mul(x, z);
    CHECK(r1->compare(*r2) == 1);
    CHECK(r2->compare(*r1) == -1);

    r1 = add(add(y, x), z);
    r2 = add(x, z);
    CHECK(r1->compare(*r2) == 1);
    CHECK(r2->compare(*r1) == -1);

    r1 = pow(x, z);
    r2 = pow(y, x);
    CHECK(r1->compare(*r2) == -1);
    CHECK(r2->compare(*r1) == 1);

    r1 = pow(x, z);
    r2 = pow(x, x);
    CHECK(r1->compare(*r2) == 1);
    CHECK(r2->compare(*r1) == -1);

    r1 = add(add(x, y), z);
    r2 = add(x, y);
    CHECK(r1->compare(*r2) == 1);
    CHECK(r2->compare(*r1) == -1);

    r1 = add(add(x, y), i2);
    r2 = add(x, y);
    CHECK(r1->compare(*r2) == 1);
    CHECK(r2->compare(*r1) == -1);

    r1 = add(add(x, y), im2);
    r2 = add(x, y);
    CHECK(r1->compare(*r2) == -1);
    CHECK(r2->compare(*r1) == 1);

    r1 = add(x, y);
    r2 = add(x, z);
    //    CHECK(r1->compare(*r2) == -1);
    //    CHECK(r2->compare(*r1) == 1);

    r1 = add(x, y);
    r2 = add(x, y);
    CHECK(r1->compare(*r2) == 0);
    CHECK(r2->compare(*r1) == 0);

    r1 = add(add(x, y), z);
    r2 = add(add(x, z), y);
    CHECK(r1->compare(*r2) == 0);
    CHECK(r2->compare(*r1) == 0);

    r1 = sin(x);
    r2 = sin(y);
    CHECK(r1->compare(*r2) == -1);
    CHECK(r2->compare(*r1) == 1);
    CHECK(r1->compare(*r1) == 0);

    r1 = real_double(1.0);
    r2 = real_double(2.0);
    CHECK(r1->compare(*r2) == -1);
    CHECK(r2->compare(*r1) == 1);
    CHECK(r1->compare(*r1) == 0);

    // These are specific to the order in the declaration of enum TypeID,
    // so we just make sure that if x < y, then y > x.
    r1 = add(x, z);
    r2 = mul(x, y);
    int cmp = r1->__cmp__(*r2);
    CHECK(cmp != 0);
    CHECK(r2->__cmp__(*r1) == -cmp);

    r1 = mul(x, pow(z, x));
    r2 = mul(x, y);
    cmp = r1->__cmp__(*r2);
    CHECK(cmp != 0);
    CHECK(r2->__cmp__(*r1) == -cmp);

    r1 = mul(x, pow(z, x));
    r2 = mul(x, z);
    cmp = r1->__cmp__(*r2);
    CHECK(cmp != 0);
    CHECK(r2->__cmp__(*r1) == -cmp);

    r1 = pow(z, x);
    r2 = pow(z, pow(x, y));
    cmp = r1->__cmp__(*r2);
    CHECK(cmp != 0);
    CHECK(r2->__cmp__(*r1) == -cmp);

    r1 = div(mul(x, y), i2);
    r2 = mul(x, y);
    cmp = r1->__cmp__(*r2);
    CHECK(cmp != 0);
    CHECK(r2->__cmp__(*r1) == -cmp);

    r1 = add(x, pow(z, x));
    r2 = add(x, y);
    cmp = r1->__cmp__(*r2);
    CHECK(cmp != 0);
    CHECK(r2->__cmp__(*r1) == -cmp);

    r1 = log(log(x));
    r2 = log(x);
    CHECK(r1->__cmp__(*r2) != 0);
    CHECK(r1->__cmp__(*r1) == 0);

    CHECK_THROWS_AS(r2->expand_as_exp(), NotImplementedError);

    r1 = pi;
    r2 = EulerGamma;
    CHECK(r1->__cmp__(*r2) > 0);
    CHECK(r1->__cmp__(*r1) == 0);
}

TEST_CASE("Complex: Basic", "[basic]")
{
    RCP<const Number> r1, r2, r3, c1, c2, c3;
    RCP<const Complex> c;
    RCP<const Basic> s;
    r1 = Rational::from_two_ints(*integer(2), *integer(4));
    r2 = Rational::from_two_ints(*integer(5), *integer(7));
    r3 = Rational::from_two_ints(*integer(-5), *integer(7));

    c1 = Complex::from_two_nums(*r1, *r2);
    c2 = Complex::from_two_nums(*r1, *r3);

    CHECK(c1->is_complex());

    // Basic check for equality in Complex::from_two_nums and
    // Complex::from_two_rats
    CHECK(eq(*c1, *Complex::from_two_rats(down_cast<const Rational &>(*r1),
                                          down_cast<const Rational &>(*r2))));
    CHECK(neq(*c2, *Complex::from_two_rats(down_cast<const Rational &>(*r1),
                                           down_cast<const Rational &>(*r2))));

    // Checks for complex addition
    // Final result is int
    CHECK(eq(*addnum(c1, c2), *one));
    // Final result is complex
    r2 = Rational::from_two_ints(*integer(1), *integer(1));
    r3 = Rational::from_two_ints(*integer(10), *integer(7));
    c3 = Complex::from_two_nums(*r2, *r3);
    CHECK(eq(*addnum(c1, c1), *c3));
    // Final result is rational
    r1 = Rational::from_two_ints(*integer(1), *integer(4));
    r2 = Rational::from_two_ints(*integer(5), *integer(7));
    r3 = Rational::from_two_ints(*integer(-5), *integer(7));
    c1 = Complex::from_two_nums(*r1, *r2);
    c2 = Complex::from_two_nums(*r1, *r3);
    CHECK(eq(*addnum(c1, c2), *div(one, integer(2))));

    // Checks for complex subtraction
    r1 = Rational::from_two_ints(*integer(2), *integer(4));
    r2 = Rational::from_two_ints(*integer(5), *integer(7));
    r3 = Rational::from_two_ints(*integer(-5), *integer(7));

    c1 = Complex::from_two_nums(*r1, *r2);
    c2 = Complex::from_two_nums(*r1, *r3);
    // Final result is int
    CHECK(eq(*subnum(c1, c1), *zero));

    // Final result is rational
    r3 = Rational::from_two_ints(*integer(1), *integer(3));
    c1 = Complex::from_two_nums(*r1, *r2);
    c2 = Complex::from_two_nums(*r3, *r2);
    CHECK(eq(*subnum(c1, c2), *div(one, integer(6))));

    // Final result is complex
    r2 = Rational::from_two_ints(*integer(1), *integer(6));
    c1 = Complex::from_two_nums(*r1, *r1);
    c2 = Complex::from_two_nums(*r3, *r3);
    c3 = Complex::from_two_nums(*r2, *r2);
    CHECK(eq(*subnum(c1, c2), *c3));

    // Checks for complex multiplication
    r1 = Rational::from_two_ints(*integer(2), *integer(1));
    r2 = Rational::from_two_ints(*integer(1), *integer(1));
    r3 = Rational::from_two_ints(*integer(-1), *integer(1));
    // Final result is int
    c1 = Complex::from_two_nums(*r1, *r2);
    c2 = Complex::from_two_nums(*r1, *r3);
    CHECK(eq(*mulnum(c1, c2), *integer(5)));

    // Final result is rational
    r1 = Rational::from_two_ints(*integer(1), *integer(2));
    c1 = Complex::from_two_nums(*r1, *r2);
    c2 = Complex::from_two_nums(*r1, *r3);
    CHECK(eq(*mulnum(c1, c2), *div(integer(5), integer(4))));

    // Final result is complex
    c1 = Complex::from_two_nums(*r2, *r2);
    c2 = Complex::from_two_nums(*r3, *r3);
    c3 = Complex::from_two_nums(*(integer(0)), *(integer(-2)));
    CHECK(eq(*mulnum(c1, c2), *c3));

    // Check for complex division
    // Final result is complex
    c1 = Complex::from_two_nums(*r2, *r2);
    c2 = Complex::from_two_nums(*r2, *r3);
    c3 = Complex::from_two_nums(*(integer(0)), *(integer(1)));
    CHECK(eq(*divnum(c1, c2), *c3));

    // Final result is integer
    c1 = Complex::from_two_nums(*r2, *r2);
    c2 = Complex::from_two_nums(*r2, *r2);
    CHECK(eq(*divnum(c1, c2), *integer(1)));

    // Final result is rational
    r3 = Rational::from_two_ints(*integer(2), *integer(1));
    c1 = Complex::from_two_nums(*r2, *r2);
    c2 = Complex::from_two_nums(*r3, *r3);
    CHECK(eq(*divnum(c1, c2), *div(integer(1), integer(2))));

    r1 = Rational::from_two_ints(*integer(1), *integer(2));
    r2 = Rational::from_two_ints(*integer(3), *integer(4));
    c1 = Complex::from_two_nums(*r1, *r2);

    r1 = Rational::from_two_ints(*integer(5), *integer(6));
    r2 = Rational::from_two_ints(*integer(7), *integer(8));
    c2 = Complex::from_two_nums(*r1, *r2);

    r1 = Rational::from_two_ints(*integer(618), *integer(841));
    r2 = Rational::from_two_ints(*integer(108), *integer(841));
    c3 = Complex::from_two_nums(*r1, *r2);
    CHECK(eq(*divnum(c1, c2), *c3));

    r1 = Rational::from_two_ints(*integer(-23), *integer(96));
    r2 = Rational::from_two_ints(*integer(17), *integer(16));
    c3 = Complex::from_two_nums(*r1, *r2);
    CHECK(eq(*mulnum(c1, c2), *c3));

    r1 = Rational::from_two_ints(*integer(4), *integer(3));
    r2 = Rational::from_two_ints(*integer(13), *integer(8));
    c3 = Complex::from_two_nums(*r1, *r2);
    CHECK(eq(*addnum(c1, c2), *c3));

    r1 = Rational::from_two_ints(*integer(-1), *integer(3));
    r2 = Rational::from_two_ints(*integer(-1), *integer(8));
    c3 = Complex::from_two_nums(*r1, *r2);
    CHECK(eq(*subnum(c1, c2), *c3));

    CHECK(is_a<Complex>(*c3));
    c = rcp_static_cast<const Complex>(c3);
    CHECK(eq(*c->real_part(), *r1));
    CHECK(eq(*c->imaginary_part(), *r2));

    // Explicit division by zero checks
    CHECK(eq(*divnum(c1, integer(0)), *ComplexInf));

    r3 = Rational::from_two_ints(*integer(0), *integer(1));
    CHECK(eq(*divnum(c1, r3), *ComplexInf));

    c2 = Complex::from_two_nums(*r3, *r3);
    CHECK(eq(*divnum(c1, c2), *ComplexInf));

    c2 = divnum(c2, integer(0));
    CHECK(eq(*c2, *Nan));
    c2 = divnum(c2, r3);
    CHECK(eq(*c2, *Nan));
    c2 = divnum(c2, c2);
    CHECK(eq(*c2, *Nan));

    c2 = Complex::from_two_nums(*integer(0), *integer(0));
    s = c2->div(*integer(0));
    CHECK(eq(*s, *Nan));
    s = c2->div(*Rational::from_two_ints(*integer(0), *integer(0)));
    CHECK(eq(*s, *Nan));
    s = c2->div(*Complex::from_two_nums(*integer(0), *integer(0)));
    CHECK(eq(*s, *Nan));
    s = integer(0)->div(*c2);
    CHECK(eq(*s, *Nan));

    c1 = Complex::from_two_nums(*integer(2), *integer(5));
    c2 = Complex::from_two_nums(*integer(1), *integer(5));
    s = c1->sub(*Rational::from_two_ints(5, 5));
    CHECK(eq(*s, *c2));

    c1 = Complex::from_two_nums(*integer(2), *integer(5));
    c2 = Complex::from_two_nums(*integer(1), *integer(5));
    s = c1->sub(*integer(1));
    CHECK(eq(*s, *c2));

    c1 = integer(1);
    c2 = Complex::from_two_nums(*integer(-1), *integer(-5));
    s = c1->sub(*Complex::from_two_nums(*integer(2), *integer(5)));
    CHECK(eq(*s, *c2));

    c1 = Complex::from_two_nums(*integer(2), *integer(5));
    c2 = Rational::from_two_ints(5, 5);
    c3 = Complex::from_two_nums(*integer(-1), *integer(-5));
    s = c2->sub(*c1);
    CHECK(eq(*s, *c3));

    c1 = Complex::from_two_nums(*integer(2), *integer(5));
    c2 = Rational::from_two_ints(5, 5);
    c3 = Complex::from_two_nums(*integer(2), *integer(5));
    s = c1->div(*c2);
    CHECK(eq(*s, *c3));

    c1 = Complex::from_two_nums(*integer(2), *integer(5));
    c2 = Rational::from_two_ints(4, 5);
    CHECK_THROWS_AS(c2->div(*c1), NotImplementedError);

    c1 = Complex::from_two_nums(*integer(2), *integer(5));
    c2 = integer(3);
    CHECK_THROWS_AS(c2->pow(*c1), NotImplementedError);
}

TEST_CASE("has_symbol: Basic", "[basic]")
{
    RCP<const Basic> r1;
    RCP<const Symbol> x, y, z;
    x = symbol("x");
    y = symbol("y");
    z = symbol("z");
    r1 = add(x, pow(y, integer(2)));
    CHECK(has_symbol(*r1, *x));
    CHECK(has_symbol(*r1, *y));
    CHECK(not has_symbol(*r1, *z));

    r1 = sin(add(x, pow(y, integer(2))));
    CHECK(has_symbol(*r1, *x));
    CHECK(has_symbol(*r1, *y));
    CHECK(not has_symbol(*r1, *z));
}

TEST_CASE("coeff: Basic", "[basic]")
{
    RCP<const Basic> r1, r2;
    RCP<const Symbol> x, y, z;
    x = symbol("x");
    y = symbol("y");
    z = symbol("z");
    r1 = add(x, pow(y, integer(2)));
    r2 = add(add(mul(integer(2), z), pow(x, integer(3))), pow(y, integer(2)));
    CHECK(eq(*coeff(*r1, *x, *integer(1)), *integer(1)));
    CHECK(eq(*coeff(*r1, *x, *integer(1)), *integer(1)));
    CHECK(eq(*coeff(*r1, *y, *integer(0)), *integer(0)));
    CHECK(eq(*coeff(*r1, *y, *integer(1)), *integer(0)));
    CHECK(eq(*coeff(*r1, *y, *integer(2)), *integer(1)));
    CHECK(eq(*coeff(*r1, *z, *integer(2)), *integer(0)));

    CHECK(eq(*coeff(*r2, *y, *integer(0)), *integer(0)));
    CHECK(eq(*coeff(*r2, *y, *integer(1)), *integer(0)));
    CHECK(eq(*coeff(*r2, *y, *integer(2)), *integer(1)));
    CHECK(eq(*coeff(*r2, *z, *integer(0)), *integer(0)));
    CHECK(eq(*coeff(*r2, *z, *integer(1)), *integer(2)));
    CHECK(eq(*coeff(*r2, *z, *integer(2)), *integer(0)));
    CHECK(eq(*coeff(*r2, *x, *integer(2)), *integer(0)));
    CHECK(eq(*coeff(*r2, *x, *integer(3)), *integer(1)));
}

TEST_CASE("free_symbols: Basic", "[basic]")
{
    RCP<const Basic> r1;
    RCP<const Symbol> x, y, z;
    x = symbol("x");
    y = symbol("y");
    z = symbol("z");
    r1 = add(x, add(z, pow(y, x)));

    set_basic s = free_symbols(*r1);
    CHECK(s.size() == 3);
    CHECK(s.count(x) == 1);
    CHECK(s.count(y) == 1);
    CHECK(s.count(z) == 1);
    s.clear();

    r1 = function_symbol("f", mul(x, integer(2)))->diff(x);
    s = free_symbols(*r1);
    CHECK(s.size() == 1);
    CHECK(s.count(x) == 1);

    r1 = mul(x, integer(2));
    s = free_symbols(*r1);
    CHECK(s.size() == 1);
    CHECK(s.count(x) == 1);
}

TEST_CASE("args: Basic", "[basic]")
{
    RCP<const Basic> r1;
    RCP<const Symbol> x, y;
    x = symbol("x");
    y = symbol("y");

    r1 = add(x, pow(y, x));
    CHECK(vec_basic_eq_perm(r1->get_args(), {x, pow(y, x)}));

    r1 = pi;
    CHECK(r1->get_args().size() == 0);

    r1 = log(pi);
    CHECK(vec_basic_eq_perm(r1->get_args(), {pi}));
}
