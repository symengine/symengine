...
TEST_CASE("Expression series expansion: Add ", "[Expansion of Add]")
{
    ...
    CHECK(series_coeff(z1, x, 9, 0)->__eq__(*integer(1)));
    CHECK(series_coeff(z1, x, 9, 1)->__eq__(*integer(0)));
}
          ...
          "[Expansion of 1/ex]")
{
    ...
    //! TODO: CHECK(expand_check_pairs(ex5, x, 8, res1));
    //! TODO: CHECK(expand_check_pairs(ex6, x, 8, res2));
}
          ...
          "[Expansion of tan, atan, asin, cot, sec, csc]")
{
    ...
    //    auto ex8 = cot(sin(x));
    //    auto ex10 = csc(x);
    //    CHECK_THROWS_AS(URatPSeriesFlint::series(ex7, "x", 10),
    //    SymEngineException);
    //    CHECK_THROWS_AS(URatPSeriesFlint::series(ex8, "x", 10),
    //    SymEngineException);
    //    CHECK_THROWS_AS(URatPSeriesFlint::series(ex10, "x", 10),
    //    SymEngineException);
}
...
TEST_CASE("Expression series expansion: lambertw ", "[Expansion of lambertw]")
{
    ...
    CHECK_THROWS_AS(URatPSeriesFlint::series(ex1, "x", 10), SymEngineException);
    CHECK_THROWS_AS(URatPSeriesFlint::series(ex2, "x", 10), SymEngineException);
}
#endif
