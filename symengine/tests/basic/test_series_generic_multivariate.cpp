...
TEST_CASE("Create MultivariateSeries", "[MultivariateSeries]")
{
    ...
    MultivariateExprPolynomial p4(MultivariatePolynomial::from_dict(s, {{v, Expression(0)}}) );
    MultivariateExprPolynomial p5(MultivariatePolynomial::from_dict(s, {{v, comp1}}) );
    ...
    // RCP<const MultivariateSeries> ms1 = make_rcp<const MultivariateSeries>( p1, "x", 5, 0, {{x, 5}, {y, 3}});
    MultivariateSeries ms1(p1, "x", 5, 0, {{x, 5}, {y, 3}});
    // RCP<const MultivariateSeries> ms2 = make_rcp<const MultivariateSeries>({ p2, "x", 5, 0, {{x, 5}, {y, 3}} });
    MultivariateSeries ms2(p2, "x", 4, 0, {{x, 4}, {y, 2}});
    // RCP<const MultivariateSeries> ms3 = make_rcp<const MultivariateSeries>({ p3, "x", 5, 0, {{x, 5}, {y, 3}} });
    MultivariateSeries ms3(p3, "x", 3, 0, {{x, 3}, {y, 5}});
    // RCP<const MultivariateSeries> ms4 = make_rcp<const MultivariateSeries>({ p4, "x", 5, 0, {{x, 5}, {y, 3}} });
    MultivariateSeries ms4(p4, "x", 2, 0, {{x, 2}, {y, 4}});
    // RCP<const MultivariateSeries> ms5 = make_rcp<const MultivariateSeries>({ p5, "x", 5, 0, {{x, 5}, {y, 3}} });
    MultivariateSeries ms5(p5, "x", 1, 0, {{x, 1}, {y, 1}});
    REQUIRE(ms1.__str__() == "2*x**2 y - b*x y**2 + a*x y - 3*y + O(|x|**5 + |y|**3)");
    REQUIRE(ms2.__str__()
            == "(-4 - f)*x**3 y**4 + (-3 + e)*x**2 y**2 + (1 + c)*x + (2 - d) + O(|x|**4 + |y|**2)");
...
TEST_CASE("Adding two UnivariateSeries", "[UnivariateSeries]")
{
    ...
    MultivariateExprPolynomial q1(MultivariatePolynomial::from_dict({x, y}, {{{2, 1}, num1},
                                                     {{1, 1}, a},
                                                     {{1, 0}, comp1 + negB},
                                                     {{0, 1}, negNum},
                                                     {{0, 0}, comp4}}) );
    ...
    map_sym_uint precisions = {{x,5}, {y,3}};
    RCP<const MultivariateSeries> ms1 = make_rcp<const MultivariateSeries>(p1, "x", 5, 0, std::move(precisions) );
    precisions = {{x, 4}, {y, 2}};
    RCP<const MultivariateSeries> ms2 = make_rcp<const MultivariateSeries>(p2, "x", 4, 0, std::move(precisions));
    precisions = {{x, 3}, {y, 5}};
    RCP<const MultivariateSeries> ms3 = make_rcp<const MultivariateSeries>(p3, "x", 3, 0, std::move(precisions));
    precisions = {{x, 2}, {y, 4}};
    RCP<const MultivariateSeries> msq1 = make_rcp<const MultivariateSeries>(q1, "x", 2, 0, std::move(precisions));
    precisions = {{x, 1}, {y, 1}};
    RCP<const MultivariateSeries> msq2 = make_rcp<const MultivariateSeries>(q2, "x", 1, 0, std::move(precisions));
    precisions = {{x, 5}, {y, 2}};
    ...
    REQUIRE(sub(ms2, ms1)->__cmp__(*neg(msq2)));
    REQUIRE(sub(ms1, ms3)->__cmp__(*ms1));
