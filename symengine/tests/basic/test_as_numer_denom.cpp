...
TEST_CASE("NumerDenom: Rational", "[as_numer_denom]")
{
    ...
    CHECK(eq(*num, *integer(2)));
    CHECK(eq(*den, *integer(5)));
    CHECK(is_a<Integer>(*num));

    as_numer_denom(r6_m2, outArg(num), outArg(den));
    CHECK(eq(*num, *integer(-3)));
    CHECK(eq(*den, *one));
    CHECK(is_a<Integer>(*num));
}
...
TEST_CASE("NumerDenom: Mul", "[as_numer_denom]")
{
    ...
    r1 = mul(x, r2_5);
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *mul(x, integer(2))));
    CHECK(eq(*den, *integer(5)));

    r1 = mul(r1, rm6_2);
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *mul(x, integer(-6))));
    CHECK(eq(*den, *integer(5)));
}
...
TEST_CASE("NumerDenom: Pow", "[as_numer_denom]")
{
    ...
    r1 = pow(r2_5, i3);
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *pow(integer(2), i3)));
    CHECK(eq(*den, *pow(integer(5), i3)));

    r1 = pow(r2_5, im3);
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *pow(integer(5), i3)));
    CHECK(eq(*den, *pow(integer(2), i3)));

    r1 = pow(r2_5, mul(im3, x));
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *pow(integer(5), mul(i3, x))));
    CHECK(eq(*den, *pow(integer(2), mul(i3, x))));
}
...
TEST_CASE("NumerDenom: Add", "[as_numer_denom]")
{
    ...
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *add(pow(x, integer(4)), one))); // x^4 + 1
    CHECK(eq(*den, *pow(x, integer(2))));           // x^2

    // (1/x^3) + (1/x^6)
    r1 = add(pow(x, integer(-3)), pow(x, integer(-6)));
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *add(pow(x, integer(3)), one))); // x^3 + 1
    CHECK(eq(*den, *pow(x, integer(6))));           // x^6

    // (x/4) + (y/6)
    r1 = add(div(x, integer(4)), div(y, integer(6)));
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *add(mul(integer(3), x), mul(integer(2), y)))); // 3*x + 2*y
    CHECK(eq(*den, *integer(12)));                                 // 12

    // (1/xy) + (1/yz)
    r1 = add(div(one, mul(x, y)), div(one, mul(y, z)));
    as_numer_denom(r1, outArg(num), outArg(den));
    CHECK(eq(*num, *add(x, z)));         // x + z
    CHECK(eq(*den, *mul(x, mul(y, z)))); // x*y*z
}
