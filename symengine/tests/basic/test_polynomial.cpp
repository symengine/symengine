...
TEST_CASE("Bool checks specific UnivariateIntPolynomial cases", "[UnivariateIntPolynomial]")
{
    ...
    REQUIRE((z->is_zero() and not z->is_one() and not z->is_minus_one() and z->is_integer() and not z->is_symbol()
...
TEST_CASE("Univariate Polynomial expand", "[UnivariatePolynomial][expand]")
{
    ...
    REQUIRE(c->__str__() == "a**3*x**9 + 3*a**2*x**8 + (2*a + a*(1 + 2*a) + a**2)*x**7 + (1 + 6*a)*x**6 + (3 + "
                            "3*a)*x**5 + 3*x**4 + x**3");
}
