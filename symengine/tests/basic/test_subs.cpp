...
TEST_CASE("Symbol: subs", "[subs]")
{
    ...
    map_basic_basic d;
    d[x] = y;
    CHECK(eq(*r1->subs(d), *r2));
    CHECK(neq(*r1->subs(d), *r1));
}
...
TEST_CASE("Mul: subs", "[subs]")
{
    ...
    r1 = div(one, mul(x, y));
    d[x] = zero;
    CHECK(eq(*r1->subs(d), *div(ComplexInf, y)));
}
...
TEST_CASE("Erf: subs", "[subs]")
{
    ...
    d.clear();
    d[z] = x;
    CHECK(eq(*r1->subs(d), *r3));

    d.clear();
    d[y] = zero;
    CHECK(eq(*r1->subs(d), *erf(x)));
}
...
TEST_CASE("LowerGamma: subs", "[subs]")
{
    ...
    map_basic_basic d;
    d[x] = one;
    CHECK(eq(*lowergamma(x, y)->subs(d), *lowergamma(one, y)));
    ...
    d[z] = zero;
    d[y] = one;
    CHECK(eq(*r1->subs(d), *lowergamma(x, one)));
...
TEST_CASE("UpperGamma: subs", "[subs]")
{
    ...
    map_basic_basic d;
    d[x] = one;
    CHECK(eq(*uppergamma(x, y)->subs(d), *uppergamma(one, y)));
    ...
    d[z] = zero;
    d[y] = one;
    CHECK(eq(*r1->subs(d), *uppergamma(x, one)));
...
TEST_CASE("PolyGamma: subs", "[subs]")
{
    ...
    map_basic_basic d;
    d[x] = one;
    CHECK(eq(*polygamma(x, y)->subs(d), *polygamma(one, y)));
    ...
    d[z] = zero;
    d[y] = one;
    CHECK(eq(*r1->subs(d), *polygamma(x, one)));
...
TEST_CASE("Beta: subs", "[subs]")
{
    ...
    map_basic_basic d;
    d[x] = one;
    CHECK(eq(*beta(x, y)->subs(d), *beta(y, one)));

    d.clear();
    d[z] = zero;
    d[y] = x;
    d[x] = y;
    CHECK(eq(*r1->subs(d), *beta(x, y)));

    d.clear();
    d[z] = zero;
    d[y] = one;
    CHECK(eq(*r1->subs(d), *beta(x, one)));

    d.clear();
    d[w] = one;
    d[z] = i2;
    d[y] = i2;
    CHECK(eq(*r1->subs(d), *beta(i2, add(i2, x))));
}
