...
TEST_CASE("Sin: functions", "[functions]")
{
    ...
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.841470984807897)
            < 1e-12);
    CHECK(std::abs(down_cast<const RealDouble &>(*r2).i + 0.416146836547142)
            < 1e-12);

    // Test is_canonical()
    RCP<const Sin> r4 = make_rcp<Sin>(i2); // dummy Sin
    CHECK(not(r4->is_canonical(zero)));
    CHECK(r4->is_canonical(i2));
    CHECK(not(r4->is_canonical(mul(pi, i2))));
    CHECK(not(r4->is_canonical(add(mul(pi, i2), div(pi, i2)))));
    CHECK(not(r4->is_canonical(real_double(2.0))));
}
...
TEST_CASE("Cos: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    REQUIRE(is_a<RealDouble>(*r2));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.540302305868140)
            < 1e-12);
    CHECK(std::abs(down_cast<const RealDouble &>(*r2).i - 0.909297426825682)
            < 1e-12);
...
TEST_CASE("Tan: functions", "[functions]")
{
    ...
    CHECK(eq(*tan(mul(integer(5), div(pi, i2))), *ComplexInf));

    r1 = tan(real_double(3.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i + 0.142546543074278)
            < 1e-12);
...
TEST_CASE("Cot: functions", "[functions]")
{
    ...
    CHECK(eq(*cot(mul(integer(7), pi)), *ComplexInf));

    r1 = cot(real_double(2.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i + 0.457657554360286)
            < 1e-12);
...
TEST_CASE("Csc: functions", "[functions]")
{
    ...
    CHECK(eq(*csc(mul(integer(7), pi)), *ComplexInf));
    CHECK(eq(*csc(integer(0)), *ComplexInf));

    r1 = csc(real_double(3.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 7.08616739573719)
            < 1e-12);
...
TEST_CASE("Sec: functions", "[functions]")
{
    ...
    CHECK(eq(*sec(mul(integer(7), div(pi, i2))), *ComplexInf));

    r1 = sec(real_double(3.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i + 1.01010866590799)
            < 1e-12);
...
TEST_CASE("TrigFunction: trig_to_sqrt", "[functions]")
{
    ...
    r1 = sin(acos(x));
    CHECK(eq(*trig_to_sqrt(r1), *sqrt(one_m_x2)));

    r1 = sin(atan(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(x, sqrt(one_p_x2))));

    r1 = sin(asec(x));
    CHECK(eq(*trig_to_sqrt(r1), *sqrt(one_m_xm2)));

    r1 = sin(acot(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, mul(x, sqrt(one_p_xm2)))));

    r1 = cos(asin(x));
    CHECK(eq(*trig_to_sqrt(r1), *sqrt(one_m_x2)));

    r1 = cos(atan(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, sqrt(one_p_x2))));

    r1 = cos(acsc(x));
    CHECK(eq(*trig_to_sqrt(r1), *sqrt(one_m_xm2)));

    r1 = cos(acot(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, sqrt(one_p_xm2))));

    r1 = tan(asin(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(x, sqrt(one_m_x2))));

    r1 = tan(acos(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(sqrt(one_m_x2), x)));

    r1 = tan(acsc(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, mul(x, sqrt(one_m_xm2)))));

    r1 = tan(asec(x));
    CHECK(eq(*trig_to_sqrt(r1), *mul(x, sqrt(one_m_xm2))));

    r1 = csc(acos(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, sqrt(one_m_x2))));

    r1 = csc(atan(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(sqrt(one_p_x2), x)));

    r1 = csc(asec(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, sqrt(one_m_xm2))));

    r1 = csc(acot(x));
    CHECK(eq(*trig_to_sqrt(r1), *mul(x, sqrt(one_p_xm2))));

    r1 = sec(asin(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, sqrt(one_m_x2))));

    r1 = sec(acos(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, x)));

    r1 = sec(atan(x));
    CHECK(eq(*trig_to_sqrt(r1), *sqrt(one_p_x2)));

    r1 = sec(acsc(x));
    CHECK(eq(*trig_to_sqrt(r1), *div(one, sqrt(one_m_xm2))));

    r1 = sec(acot(x));
    CHECK(eq(*trig_to_sqrt(r1), *sqrt(one_p_xm2)));
...
TEST_CASE("Derivative: functions", "[functions]")
{
    ...
    CHECK(neq(*r1, *r3));
    CHECK(neq(*r2, *r3));
    CHECK(unified_eq(r1->get_args(), {f, x}));

    r1 = f->diff(x)->diff(x);
    r2 = Derivative::create(f, {x, x});
    CHECK(eq(*r1, *r2));
    CHECK(unified_eq(r1->get_args(), {f, x, x}));
    ...
    CHECK(eq(*r1, *mul(mul(integer(2), x), r2)));
    ...
    CHECK(r4->is_canonical(function_symbol("f", {y, x}), {x}));
    CHECK(not r4->is_canonical(function_symbol("f", y), {x}));
    CHECK(not r4->is_canonical(function_symbol("f", x), {x, y, x, x}));
    CHECK(not(r4->is_canonical(function_symbol("f", x), {pow(x, integer(2))})));

    // Test get_args()
    r1 = Derivative::create(function_symbol("f", {x, y, pow(z, integer(2))}),
                            {x, x, y});
    CHECK(vec_basic_eq_perm(
        r1->get_args(),
...
TEST_CASE("Get pi shift: functions", "[functions]")
{
    ...
    CHECK(b == true);
    CHECK(eq(*n, *integer(2)));
    CHECK(eq(*r1, *i3));

    // arg = n*pi/12
    r = mul(pi, div(one, integer(12)));
    get_pi_shift(r, outArg(n), outArg(r1));
    CHECK(eq(*n, *div(one, integer(12))));
    CHECK(eq(*r1, *zero));

    // arg = 2*pi/3
    r = mul(pi, div(i2, integer(3)));
    b = get_pi_shift(r, outArg(n), outArg(r1));
    CHECK((eq(*n, *div(i2, integer(3))) and (b == true) and eq(*r1, *zero)));

    // arg = 2 * pi / 5
    r = mul(pi, div(i2, integer(5)));
    b = get_pi_shift(r, outArg(n), outArg(r1));
    CHECK(((b == true) and eq(*n, *div(i2, integer(5)))));
    ...
    b = get_pi_shift(r, outArg(n), outArg(r1));
    CHECK(b == true);
    CHECK(eq(*n, *div(i2, integer(3))));
    CHECK(eq(*r1, *mul(i2, x)));
    ...
    CHECK(((b == true) and eq(*n, *one) and eq(*r1, *zero)));
    ...
    CHECK(b == true);
    CHECK(eq(*n, *div(i2, integer(3))));
    CHECK(eq(*r1, *add(mul(i2, x), mul(i2, symbol("y")))));
}
...
TEST_CASE("Asin: functions", "[functions]")
{
    ...
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.523598775598299)
          < 1e-12);
    ...
    CHECK(not(r4->is_canonical(zero)));
    CHECK(not(r4->is_canonical(one)));
    CHECK(not(r4->is_canonical(minus_one)));
    CHECK(r4->is_canonical(i2));
    CHECK(not(r4->is_canonical(real_double(2.0))));
}
...
TEST_CASE("Acos: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 1.04719755119660)
            < 1e-12);
...
TEST_CASE("Asec: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 2.09439510239320)
            < 1e-12);
...
TEST_CASE("atan: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 1.24904577239825)
            < 1e-12);
...
TEST_CASE("Acot: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.463647609000806)
            < 1e-12);
...
TEST_CASE("Atan2: functions", "[functions]")
{
    ...
    r1 = atan2(y, x);
    CHECK(unified_eq(r1->get_args(), {y, x}));
    ...
    CHECK(not(r4->is_canonical(zero, i2)));
    CHECK(not(r4->is_canonical(zero, zero)));
    CHECK(not(r4->is_canonical(i2, i2)));
    CHECK(not(r4->is_canonical(i2, neg(i2))));
    CHECK(r4->is_canonical(i2, i3));
    CHECK(not(r4->is_canonical(one, sqrt(i3))));
}
...
TEST_CASE("Lambertw: functions", "[functions]")
{
    ...
    CHECK(not(r4->is_canonical(zero)));
    CHECK(not(r4->is_canonical(E)));
    CHECK(not(r4->is_canonical(neg(div(one, E)))));
    CHECK(not(r4->is_canonical(div(log(i2), im2))));
    CHECK(r4->is_canonical(i2));
}
...
TEST_CASE("Sinh: functions", "[functions]")
{
    ...
    CHECK(eq(*sinh(sub(x, y)), *neg(sinh(sub(y, x)))));

    r1 = sinh(real_double(1.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 1.1752011936438)
            < 1e-12);

    RCP<const Sinh> r4 = make_rcp<Sinh>(i2);
    CHECK(not(r4->is_canonical(zero)));
    CHECK(r4->is_canonical(one));
    CHECK(not(r4->is_canonical(minus_one)));
    CHECK(not(r4->is_canonical(neg(i2))));
    CHECK(not(r4->is_canonical(neg(x))));
    CHECK(r4->is_canonical(i2));
    CHECK(not(r4->is_canonical(real_double(2.0))));
}
...
TEST_CASE("Csch: functions", "[functions]")
{
    ...
    CHECK(eq(*csch(sub(x, y)), *neg(csch(sub(y, x)))));

    r1 = csch(real_double(1.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.850918128239322)
            < 1e-12);
...
TEST_CASE("Cosh: functions", "[functions]")
{
    ...
    CHECK(eq(*r1, *r2));
    CHECK(eq(*cosh(sub(x, y)), *cosh(sub(y, x))));

    r1 = cosh(real_double(2.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 3.76219569108363)
            < 1e-12);
...
TEST_CASE("Sech: functions", "[functions]")
{
    ...
    CHECK(eq(*r1, *r2));
    CHECK(eq(*sech(sub(x, y)), *sech(sub(y, x))));

    r1 = sech(real_double(4.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.0366189934736865)
            < 1e-12);
...
TEST_CASE("Tanh: functions", "[functions]")
{
    ...
    CHECK(eq(*tanh(sub(x, y)), *neg(tanh(sub(y, x)))));

    r1 = tanh(real_double(2.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.964027580075817)
            < 1e-12);
...
TEST_CASE("Coth: functions", "[functions]")
{
    ...
    CHECK(eq(*coth(sub(x, y)), *neg(coth(sub(y, x)))));

    r1 = coth(real_double(2.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 1.03731472072755)
            < 1e-12);
...
TEST_CASE("Asinh: functions", "[functions]")
{
    ...
    CHECK(eq(*asinh(sub(x, y)), *neg(asinh(sub(y, x)))));

    r1 = asinh(real_double(3.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 1.81844645923207)
            < 1e-12);
...
TEST_CASE("Acsch: functions", "[functions]")
{
    ...
    CHECK(eq(*acsch(sub(x, y)), *neg(acsch(sub(y, x)))));

    r1 = acsch(real_double(2.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.481211825059603)
            < 1e-12);
...
TEST_CASE("Acosh: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 1.31695789692482)
            < 1e-12);
...
TEST_CASE("Atanh: functions", "[functions]")
{
    ...
    CHECK(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.549306144334055)
          < 1e-12);
    ...
    CHECK(eq(*atanh(sub(x, y)), *neg(atanh(sub(y, x)))));
...
TEST_CASE("Acoth: functions", "[functions]")
{
    ...
    CHECK(eq(*acoth(sub(x, y)), *neg(acoth(sub(y, x)))));

    r1 = acoth(real_double(3.0));
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.346573590279973)
            < 1e-12);
...
TEST_CASE("Kronecker Delta: functions", "[functions]")
{
    ...
    CHECK(not(r4->is_canonical(i2, i2)));
    CHECK(not(r4->is_canonical(i, i)));
    CHECK(not(r4->is_canonical(i2, add(i2, i2))));
    CHECK(r4->is_canonical(i, j));
}
...
TEST_CASE("Zeta: functions", "[functions]")
{
    ...
    CHECK(r1->__str__() == "zeta(3, 2)");

    r1 = zeta(x, i2);
    CHECK(r1->__str__() == "zeta(x, 2)");
    ...
    CHECK(not(r4->is_canonical(zero, i2)));
    CHECK(not(r4->is_canonical(one, i2)));
    CHECK(not(r4->is_canonical(i2, add(i2, i2))));
    CHECK(r4->is_canonical(x, y));
}
...
TEST_CASE("Levi Civita: functions", "[functions]")
{
    ...
    CHECK(not r4->is_canonical({i2, i2}));
    CHECK(not(r4->is_canonical({i2, i, i})));
    CHECK(r4->is_canonical({i2, i}));
}
...
TEST_CASE("Erf: functions", "[functions]")
{
    ...
    CHECK(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.84270079294971)
          < 1e-12);
    ...
    CHECK(eq(*erf(neg(x)), *neg(erf(x))));
    CHECK(eq(*erf(sub(x, y)), *neg(erf(sub(y, x)))));
...
TEST_CASE("Erfc: functions", "[functions]")
{
    ...
    CHECK(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 0.15729920705028)
          < 1e-12);
    ...
    CHECK(eq(*r1->diff(x), *neg(r2)));

    CHECK(eq(*erfc(neg(x)), *sub(i2, erfc(x))));
    CHECK(eq(*erfc(sub(y, x)), *sub(i2, erfc(sub(x, y)))));
...
TEST_CASE("Gamma: functions", "[functions]")
{
    ...
    REQUIRE(is_a<RealDouble>(*r1));
    CHECK(std::abs(down_cast<const RealDouble &>(*r1).i - 4.17065178379660)
            < 1e-12);
    ...
    CHECK(not(r4->is_canonical(i2)));
    CHECK(r4->is_canonical(y));
    CHECK(not(r4->is_canonical(div(one, i2))));
    CHECK(not(r4->is_canonical(real_double(2.0))));
}
...
TEST_CASE("LogGamma: functions", "[functions]")
{
    ...
    CHECK(not(r4->is_canonical(minus_one)));
    CHECK(not(r4->is_canonical(one)));
    CHECK(not(r4->is_canonical(integer(2))));
    CHECK(not(r4->is_canonical(integer(3))));
}
...
TEST_CASE("Lowergamma: functions", "[functions]")
{
    ...
    CHECK(not(r4->is_canonical(one, x)));
    CHECK(not(r4->is_canonical(i2, y)));
    CHECK(not(r4->is_canonical(div(one, i2), i2)));
}
...
TEST_CASE("Abs: functions", "[functions]")
{
    ...
    CHECK(eq(*abs(add(i2, mul(I, im1))), *sqrt(integer(5))));
    CHECK(eq(*abs(add(i2, mul(I, i3))), *sqrt(integer(13))));
    CHECK(eq(*abs(x), *abs(neg(x))));
    CHECK(eq(*abs(one), *one));
    CHECK(eq(*abs(i2), *i2));
    CHECK(eq(*abs(im1), *one));
    CHECK(eq(*abs(integer(-5)), *integer(5)));
    CHECK(neq(*abs(sqrt_pi), *sqrt_pi));
    CHECK(eq(*abs(sqrt_pi), *abs(sqrt_pi)));
    CHECK(eq(*abs(div(i2, i3)), *div(i2, i3)));
    CHECK(eq(*abs(neg(div(i2, i3))), *div(i2, i3)));
    CHECK(neq(*abs(x)->diff(x), *integer(0)));
    CHECK(eq(*abs(x)->diff(y), *integer(0)));
    CHECK(eq(*abs(sub(x, y)), *abs(sub(y, x))));
    CHECK(eq(*abs(real_double(-1.0)), *real_double(1.0)));
}
...
TEST_CASE("FunctionWrapper: functions", "[functions]")
{
    ...
    CHECK(std::fabs(d - 1.84147098480789) < 1e-12);

    CHECK(e->__str__() == "1 + MySin(x)");

#ifdef HAVE_SYMENGINE_MPFR
    mpfr_class aa(100);
    eval_mpfr(aa.get_mpfr_t(), *f, MPFR_RNDN);
    CHECK(mpfr_cmp_d(aa.get_mpfr_t(), 1.8414709848078) == 1);
    CHECK(mpfr_cmp_d(aa.get_mpfr_t(), 1.8414709848079) == -1);
#ifdef HAVE_SYMENGINE_MPC
    mpc_class a(100);
...
TEST_CASE("MPFR and MPC: functions", "[functions]")
{
    ...
    CHECK(is_a<RealMPFR>(*r1));

    mpfr_mul_z(a.get_mpfr_t(), down_cast<const RealMPFR &>(*r1).i.get_mpfr_t(),
               get_mpz_t(p), MPFR_RNDN);
    q = 0_z;
    CHECK(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(q)) == 0);

    mpfr_set_si(a.get_mpfr_t(), -22, MPFR_RNDN);
    r1 = abs(real_mpfr(a));
    q = 22_z;
    CHECK(mpfr_cmp_z(down_cast<const RealMPFR &>(*r1).i.get_mpfr_t(),
                       get_mpz_t(q))
            == 0);
    ...
    for (unsigned i = 0; i < testvec.size(); i++) {
        ...
        mpfr_mul_z(a.get_mpfr_t(),
                   ...
                   get_mpz_t(p), MPFR_RNDN);
        CHECK(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(std::get<1>(testvec[i])))
                > 0);
        CHECK(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(std::get<2>(testvec[i])))
                < 0);
    }
    testvec.clear();
    ...
    mpfr_mul_z(a.get_mpfr_t(), down_cast<const RealMPFR &>(*r1).i.get_mpfr_t(),
               get_mpz_t(p), MPFR_RNDN);
    CHECK(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(3605551275463989_z)) == 1);
    CHECK(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(3605551275463990_z)) == -1);
...
TEST_CASE("max: functions", "[functions]")
{
    ...
    res = max({y, x});
    CHECK(eq(*(down_cast<const Max &>(*res).get_args()[0]), *tmp));

    res = max({x, y});
    CHECK(eq(*res, *max({y, x}))); // max(x, y) == max(y, x)
    CHECK(is_a<Max>(*res));        // max(x, y) is a Max

    res = max({x});
    CHECK(eq(*res, *x)); // max(x) == x
    ...
    res = max({x, x});
    CHECK(eq(*res, *x)); // max(x, x) == x

    res = max({x, x, max({x, y})});
    CHECK(eq(*res, *max({x, y}))); // max(x, x, max(x, y)) == max(x,y)

    res = max({i2, rd, r2_5});
    CHECK(eq(*res, *i2)); // max(2, 2/5, 0.32) == 2

    res = max({x, max({i2, y})});
    CHECK(eq(*res, *max({x, i2, y}))); // max(x, max(2, y)) == max(x, 2, y)
    ...
    res = max({i2, r2_5, x});
    CHECK(eq(*res, *max({i2, x}))); // max(2, 2/5, x) == max(2, x)

    res = max({max({x, i2}), max({y, r2_5})});
    CHECK(eq(*res,
             *max({x, i2, y}))); // max(max(2, x), max(2/5, y)) == max(x, 2, y)
...
TEST_CASE("min: functions", "[functions]")
{
    ...
    res = min({x, y});
    CHECK(eq(*res, *min({y, x}))); // min(x, y) == min(y, x)
    CHECK(is_a<Min>(*res));        // min(x, y) is a min

    res = min({x});
    CHECK(eq(*res, *x)); // min(x) == x

    res = min({x, x});
    CHECK(eq(*res, *x)); // min(x, x) == x

    res = min({x, x, min({x, y})});
    CHECK(eq(*res, *min({x, y}))); // min(x, x, min(x, y)) == min(x,y)

    res = min({i2, rd, r2_5});
    CHECK(eq(*res, *rd)); // min(2, 2/5, 0.32) == 0.32

    res = min({i2, rd, max({x})});
    CHECK(eq(*res, *min({rd, x}))); // min(2, 0.32, max(x)) == min(0.32, x)

    res = min({x, min({i2, y})});
    CHECK(eq(*res, *min({x, i2, y}))); // min(x, min(2, y)) == min(x, 2, y)
    ...
    CHECK(eq(*res, *min({x, r2_5,
                         y}))); // min(min(2, x), min(2/5, y)) == min(x, 2/5, y)
