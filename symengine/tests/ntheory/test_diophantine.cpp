...
bool vec_dense_matrix_eq_perm(const std::vector<DenseMatrix> &a, const std::vector<DenseMatrix> &b)
{
...
TEST_CASE("test_homogeneous_lde()", "[diophantine]")
{
    ...
    homogeneous_lde(basis, A);
    true_basis = std::vector<DenseMatrix>{DenseMatrix(1, 4, {integer(0), integer(0), integer(3), integer(2)}),
    ...
    A = DenseMatrix(1, 2, {integer(2), integer(-3)});
    homogeneous_lde(basis, A);
    true_basis = std::vector<DenseMatrix>{DenseMatrix(1, 2, {integer(3), integer(2)})};
