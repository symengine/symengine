...
class ExpressionParser
{
    ...
    // cast overlaoded functions below to single_arg, double_arg before
    // they can
    // be used in the map
    ...
    // eg. for a '(', it stores where the next ',' or ')' occurs, so as
    // to know
    // what part of the string is to be parsed 'together'
    std::vector<int> operator_end;

    // the string to be parsed, obtained after removing all spaces from
    // input
    // string
    std::string s;
    // it's length
    ...
    RCP<const Basic> parse_string(unsigned int l, unsigned int h)
    {
        ...
        for (unsigned int iter = l; iter < h; ++iter) {
            if (is_operator(iter)) {
                // if an operator is encountered, which is not '(' a
                // result must
                ...
                // continue the parsing after operator_end[iter], as we
                // have
                ...
                // if the parsing was to finish after this, result must
                // be set
...
public:
    ...
    {
        // stack to maintain right brackets, to match with corresponding
        // left
        // brackets
            ...
            if (is_operator(i)) {
                ...
                if (x == '(') {
                    ...
                    while (op_stack.top().second != right_bracket.top())
                        op_stack.pop();
                    // it's end is the index of the ')' (maybe pseudo
                    // created by
                    // a ',')
                    ...
                    // if it's a normal operator, remove operators with
                    // higher
                    // precedence
