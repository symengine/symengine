...
namespace SymEngine
{
#define modFunction [&](integer_class a) { \
    integer_class temp = a % this->modulo_; \
    if (temp < 0) \
        temp += this->modulo_; \
    return temp; \
}
...
public:
    ...
    // {
    // }
    GaloisFieldDict(const int &i, const integer_class &mod) : ODictWrapper(i, modFunction), modulo_(mod)
    {
    }
    GaloisFieldDict(const map_uint_mpz &p, const integer_class &mod) : ODictWrapper(p, modFunction), modulo_(mod)
    {
    }
    GaloisFieldDict(const integer_class &i, const integer_class &mod) : ODictWrapper(i, modFunction), modulo_(mod)
    {
    }
    ...
    // RCP<const GaloisField> gf_lcm(const RCP<const GaloisField> &o) const;
};
...
public:
    map_uint_mpz dict_; //temporary
    integer_class modulo_; // temporary
