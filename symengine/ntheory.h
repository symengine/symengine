...
// Sieve class stores all the primes upto a limit. When a prime or a
// list of
// prime
// is requested, if the prime is not there in the sieve, it is extended
// to hold
// that
// prime. The implementation is a very basic Eratosthenes sieve, but the
// code
// should
...
public:
    // Returns all primes up to the `limit` (including). The vector
    // `primes`
    // should
    ...
    static void clear();
    // Set the sieve size in kilobytes. Set it to L1d cache size for
    // best
    // performance.
    // Default value is 32.
    static void set_sieve_size(unsigned size);
    // Set whether the sieve is cleared after the sieve is extended in
    // internal
    // functions
...
//! Computes the Bernoulli number Bn as an exact fraction, for an
//! isolated
//! integer n
...
//! Computes a primitive root. Returns false if no primitive root
//! exists.
...
//! Computes all primitive roots less than n. Returns false if no
//! primitive root
//! exists.
...
bool nthroot_mod(const Ptr<RCP<const Integer>> &root,
                 const RCP<const Integer> &a, const RCP<const Integer> &n,
                 const RCP<const Integer> &m);
//! A solution to x**s == a**r mod m where b = r / s. Return false if
//! none
//! exists.
...
//! All solutions to x**s == a**r mod m where b = r / s. Return false if
//! none
//! exists.
...
//! Mobius Function
// mu(n) = 1 if n is a square-free positive integer with an even number
// of prime
// factors
// mu(n) = −1 if n is a square-free positive integer with an odd number
// of prime
// factors
