...
public:
    ...
    // dictionary:
    Mul(const Teuchos::RCP<Rational> &coef, const map_basic_basic& dict);
    virtual std::size_t __hash__() const;
    virtual bool __eq__(const Basic &o) const;
    virtual std::string __str__() const;

    void as_coef_term(const Teuchos::Ptr<Teuchos::RCP<Rational>> &coef,
            const Teuchos::Ptr<Teuchos::RCP<Basic>> &term);
    // Performs canonicalization first:
    static Teuchos::RCP<Basic> from_dict(const Teuchos::RCP<Rational> &coef,
